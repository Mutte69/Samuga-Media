...
let contentLabPendingRefresh = false;
...
function resetSocialCardCreator(clearDraft = true) {
  currentSocialCard = null;
  $("#socialCardForm")?.reset();
  if ($("#socialCardCategory")) $("#socialCardCategory").value = "LOCAL";
  $("#socialCardError").textContent = "";
  $("#socialCardState").textContent = "Not created";
  $("#socialCardGenerated").innerHTML = `<span>Your generated card appears here.</span>`;
  $("#socialCardPostActions").hidden = true;
  renderSocialCardPhotoPreview();
  if (clearDraft) localStorage.removeItem("samuga-social-card-draft-v1");
}

function renderSocialCardPhotoPreview() {
  const url = $("#socialCardImageUrl")?.value.trim() || "";
  $("#removeSocialCardImageBtn").hidden = !url;
  $("#socialCardPhotoPreview").innerHTML = url ? `<img src="${attr(url)}" alt="Selected background">` : `<span>No background photo selected</span>`;
  try { localStorage.setItem("samuga-social-card-draft-v1", JSON.stringify({headline:$("#socialCardHeadline")?.value||"",paragraph:$("#socialCardParagraph")?.value||"",category:$("#socialCardCategory")?.value||"LOCAL",image_url:url})); } catch {}
}

async function uploadSocialCardImage(file) {
  if (!file) return;
  try {
    const data = await uploadFile(file, true);
    if (data.type !== "image") throw new Error("Social card backgrounds must be images.");
    $("#socialCardImageUrl").value = data.url; renderSocialCardPhotoPreview(); toast("Background photo uploaded");
  } catch (error) { toast(error.message, true); }
  finally { $("#socialCardImageUpload").value = ""; }
}
function removeSocialCardImage() { $("#socialCardImageUrl").value = ""; renderSocialCardPhotoPreview(); }

async function createSocialCard(event) {
  event?.preventDefault(); event?.stopPropagation();
  const payload = {headline:$("#socialCardHeadline").value.trim(),paragraph:$("#socialCardParagraph").value.trim(),category:$("#socialCardCategory").value,image_url:$("#socialCardImageUrl").value.trim()};
  if (!payload.headline) { $("#socialCardError").textContent = "Headline is required."; return; }
  const button = $("#createSocialCardBtn"); button.disabled = true; button.textContent = "Creating…"; $("#socialCardError").textContent = "";
  try {
    const data = await api("/api/admin/social-card/create", {method:"POST", body:JSON.stringify(payload)});
    currentSocialCard = data.card;
    $("#socialCardState").textContent = "Ready";
...
async function postSocialCard(destination) {
  if (!currentSocialCard?.card_id) { toast("Create a card first.", true); return; }
  const buttons = $$('[data-social-card-post]'); buttons.forEach(button => button.disabled = true);
  $("#socialCardState").textContent = "Publishing…";
  try {
    const data = await api("/api/admin/social-card/post", {method:"POST", body:JSON.stringify({card_id:currentSocialCard.card_id,destination})});
    $("#socialCardState").textContent = data.status || "Queued";
    toast(data.message || "Publishing started"); loadSocialCards();
  } catch (error) { $("#socialCardState").textContent = "Failed"; toast(error.message, true); }
  finally { buttons.forEach(button => button.disabled = false); }
}

async function loadSocialCards() {
  const container = $("#socialCardHistory"); if (!container) return;
  try {
    ...
    $$('[data-reuse-social-card]', container).forEach(button => button.addEventListener("click", () => {
      ...
      renderSocialCardPhotoPreview(); $("#socialCardGenerated").innerHTML = `<img src="${attr(card.card_url)}" alt="Generated card">`; $("#socialCardPostActions").hidden = false; $("#socialCardState").textContent = card.status || "Ready"; window.scrollTo({top:0,behavior:"smooth"});
    }));
  } catch (error) { container.innerHTML = `<div class="empty-panel error">${esc(error.message)}</div>`; }
}

function analyticsList(items, total) {
  const max = Math.max(1, ...items.map(item => Number(item.views || 0)));
  return items.length ? items.map(item => `<div class="analytics-list-row"><div><strong>${esc(String(item.name || "Unknown"))}</strong><span>${Number(item.views || 0).toLocaleString()}</span></div><i style="width:${Math.max(3,Number(item.views||0)/max*100)}%"></i></div>`).join("") : `<div class="empty-panel">No traffic recorded yet.</div>`;
}
...
async function loadAnalytics() {
  ...
  try {
    ...
    $("#analyticsViews").textContent = Number(totals.views || 0).toLocaleString();
    $("#analyticsVisitors").textContent = Number(totals.visitors || 0).toLocaleString();
    $("#analyticsTodayViews").textContent = Number(totals.today_views || 0).toLocaleString();
    $("#analyticsTodayVisitors").textContent = Number(totals.today_visitors || 0).toLocaleString();

    const coverage = data.coverage || {};
    const first = coverage.first_event_at ? new Date(coverage.first_event_at) : null;
    const last = coverage.last_event_at ? new Date(coverage.last_event_at) : null;
    const validFirst = first && !Number.isNaN(first.getTime());
    const validLast = last && !Number.isNaN(last.getTime());
    $("#analyticsCoverage").textContent = validFirst
      ? `Tracking since ${first.toLocaleString([], {dateStyle:"medium", timeStyle:"short"})} · ${Number(coverage.lifetime_events || 0).toLocaleString()} lifetime pageviews${validLast ? ` · last event ${last.toLocaleTimeString([], {hour:"2-digit",minute:"2-digit"})}` : ""}`
      : "Waiting for the first tracked public page load.";
    $("#analyticsComparison").textContent = data.comparison_note || "Cloudflare and Samuga analytics are separate measurement systems.";

    const cloudflare = data.cloudflare || {};
    const cloudflareStatus = $("#cloudflareAnalyticsStatus");
    const cloudflareMessage = $("#cloudflareAnalyticsMessage");
    const cloudflareStats = $("#cloudflareAnalyticsStats");
    if (cloudflare.status === "ok") {
      const cfTotals = cloudflare.totals || {};
      cloudflareStatus.textContent = "Connected";
      cloudflareStatus.className = "status-pill posted";
      cloudflareMessage.textContent = cloudflare.message || "Cloudflare historical edge metrics loaded.";
      cloudflareStats.hidden = false;
      $("#cloudflareRequests").textContent = Number(cfTotals.requests || 0).toLocaleString();
      $("#cloudflarePageviews").textContent = Number(cfTotals.pageviews || 0).toLocaleString();
      $("#cloudflareUniques").textContent = Number(cfTotals.peak_daily_unique_visitors || 0).toLocaleString();
      const cfRange = cloudflare.range || {};
      $("#cloudflareRange").textContent = cfRange.start && cfRange.end ? `${cfRange.start.slice(5)} – ${cfRange.end.slice(5)}` : `${days} days`;
    } else {
      cloudflareStats.hidden = true;
      cloudflareStatus.textContent = cloudflare.status === "error" ? "Connection error" : "Optional";
      cloudflareStatus.className = `status-pill ${cloudflare.status === "error" ? "failed" : "draft"}`;
      cloudflareMessage.textContent = cloudflare.message || "Add CLOUDFLARE_ZONE_ID and CLOUDFLARE_ANALYTICS_TOKEN in Railway.";
    }

    const daily = data.daily || [];
    const max = Math.max(1, ...daily.map(item => Number(item.views || 0)));
    const todayIso = new Date().toLocaleDateString("en-CA");
    $("#analyticsTrend").innerHTML = daily.length ? daily.map(item => {
      const views = Number(item.views || 0), visitors = Number(item.visitors || 0);
      const viewHeight = views ? Math.max(5, views / max * 100) : 0;
      const visitorHeight = visitors ? Math.max(4, visitors / max * 100) : 0;
      const labelDate = new Date(`${item.day}T00:00:00`);
      const label = Number.isNaN(labelDate.getTime()) ? item.day.slice(5) : labelDate.toLocaleDateString([], {month:"short",day:"numeric"});
      return `<div class="analytics-day ${views ? "" : "zero"} ${item.day === todayIso ? "today" : ""}" title="${attr(item.day)} · ${views} pageviews · ${visitors} browsers"><div><i style="height:${viewHeight}%"></i><b style="height:${visitorHeight}%"></b></div><span>${esc(label)}</span></div>`;
    }).join("") : `<div class="empty-panel">Traffic appears as public pages are viewed.</div>`;

    const pages = data.top_pages || [];
    $("#analyticsTopPages").innerHTML = pages.length ? `<div class="analytics-table-row head"><span>Page</span><span>Views</span><span>Visitors</span></div>` + pages.map(item => `<div class="analytics-table-row"><span><strong>${esc(item.label)}</strong><small>${esc(item.path||"")}</small></span><span>${Number(item.views||0).toLocaleString()}</span><span>${Number(item.visitors||0).toLocaleString()}</span></div>`).join("") : `<div class="empty-panel">No pageviews recorded in this range.</div>`;
    $("#analyticsDevices").innerHTML = analyticsList(data.devices || []);
    $("#analyticsReferrers").innerHTML = analyticsList(data.referrers || []);
    $("#analyticsLanguages").innerHTML = analyticsList(data.languages || []);
    $("#analyticsPrivacy").textContent = data.privacy || "";
  } catch (error) { toast(error.message, true); }
}

const AI_PROVIDER_ORDER = ["Claude", "Gemini", "OpenAI", "DeepSeek"];
let aiUsageRefreshTimer = null;
let aiUsageStreamOffset = 0;
let aiUsageStreamTotal = 0;
let aiUsageStreamLoading = false;
let providerDiagnostics = null;

const aiFmtNumber = value => Number(value || 0).toLocaleString("en-US");
const aiFmtCost = value => new Intl.NumberFormat("en-US", {style:"currency", currency:"USD", minimumFractionDigits:Number(value || 0) < 0.01 ? 4 : 2, maximumFractionDigits:6}).format(Number(value || 0));
function aiFmtDuration(ms) {
  const n = Number(ms || 0);
  if (n < 1000) return `${Math.round(n)} ms`;
  if (n < 60000) return `${(n / 1000).toFixed(n < 10000 ? 1 : 0)} s`;
  return `${Math.floor(n / 60000)}m ${Math.round((n % 60000) / 1000)}s`;
}
function aiStatusClass(status) { return status === "success" ? "posted" : status === "pending" ? "scheduled" : status === "blocked" ? "draft" : "failed"; }
function aiDateTime(value) {
  const date = value ? new Date(value) : null;
  return date && !Number.isNaN(date.getTime()) ? date.toLocaleString([], {month:"short", day:"numeric", hour:"2-digit", minute:"2-digit", second:"2-digit"}) : "—";
}
function aiShortModel(value) {
...
function aiBarWidth(value, max) { return Number(value || 0) ? Math.max(3, Number(value || 0) / Math.max(1, Number(max || 1)) * 100) : 0; }
function setSelectOptions(select, values, firstLabel) {
  if (!select) return;
  const current = select.value;
  select.innerHTML = `<option value="">${esc(firstLabel)}</option>` + (values || []).map(value => `<option value="${attr(value)}">${esc(value)}</option>`).join("");
  if ([...select.options].some(option => option.value === current)) select.value = current;
}

async function loadAIUsage(resetStream = true) {
  const days = Number($("#aiUsageDays")?.value || 30);
  try {
    const [data, diagnostics] = await Promise.all([
      api(`/api/admin/ai-usage?days=${days}`),
      api("/api/admin/provider-diagnostics"),
    ]);
...
function providerStatusPill(disabled, enabledByEnv=true) {
  if (!enabledByEnv) return {text:"Disabled by Railway", cls:"failed"};
  if (disabled) return {text:"Stopped", cls:"failed"};
  return {text:"Active", cls:"posted"};
}

function renderProviderDiagnostics(data) {
  const gemini = data.gemini || {}, gi = gemini.identity || {}, gr = gemini.runtime || {}, gu = gemini.usage || {}, gl = gemini.limits || {};
  const gs = providerStatusPill(Boolean(gr.disabled), gemini.enabled_by_env !== false);
  const gStatus = $("#geminiGuardStatus"); if (gStatus) { gStatus.textContent = gs.text; gStatus.className = `status-pill ${gs.cls}`; }
  const verifiedProject = gi.lookup_status === "verified" ? (gi.lookup_project_number || gi.lookup_key_resource || "Verified") : `Not verified (${gi.lookup_status || "unknown"})`;
  $("#geminiGuardBody").innerHTML = `<div class="provider-facts">
    <span><small>API key</small><strong>${esc(gi.key_masked || "not configured")}</strong><em>Fingerprint ${esc(gi.key_fingerprint || "—")}</em></span>
    <span><small>Configured project</small><strong>${esc(gi.configured_project || "Not supplied")}</strong><em>${esc(verifiedProject)}</em></span>
    <span><small>Requests</small><strong>${aiFmtNumber(gu.hour)} hour · ${aiFmtNumber(gu.day)} today</strong><em>10-minute window ${aiFmtNumber(gu.ten_min)}</em></span>
    <span><small>Estimated Gemini cost today</small><strong>${aiFmtCost(gu.day_cost)}</strong><em>Automatic stop at ${aiFmtCost(gl.cost_usd_per_day)}</em></span>
    <span><small>Safety limits</small><strong>${aiFmtNumber(gl.requests_per_hour)}/hour · ${aiFmtNumber(gl.requests_per_day)}/day</strong><em>${aiFmtNumber(gl.max_retries)} retries · ${aiFmtNumber(gl.requests_per_story_per_hour)}/story/hour</em></span>
    <span><small>Runtime state</small><strong>${gr.disabled ? esc(gr.reason || "Disabled") : "Accepting guarded requests"}</strong><em>${gr.disabled_until ? `Until ${esc(aiDateTime(gr.disabled_until))}` : "No active cooldown"}</em></span>
  </div><div class="provider-mini-breakdown"><strong>Gemini 30-day attribution</strong>${(gemini.by_feature_30d || []).slice(0,6).map(item => `<span>${esc(item.feature)}<b>${aiFmtNumber(item.requests)} · ${aiFmtCost(item.cost)} · ${aiFmtNumber(item.retry_count)} retries</b></span>`).join("") || `<span>No detailed telemetry yet.</span>`}${(gemini.by_key_30d || []).map(item => `<span>Key ${esc(item.key_masked || item.key_fingerprint)}<b>${aiFmtNumber(item.requests)} requests · ${aiFmtCost(item.cost)}</b></span>`).join("")}</div>`;
  const toggleGemini = $("#toggleGeminiBtn"); if (toggleGemini) toggleGemini.textContent = gr.disabled ? "Enable Gemini" : "Disable Gemini";

  const buffer = data.buffer || {}, bi = buffer.identity || {}, br = buffer.runtime || {}, bc = buffer.counts || {}, bl = buffer.limits || {}, rate = buffer.official_rate_limit_remaining || {}, snap = buffer.account_snapshot || {}, plan = buffer.plan_evidence || snap.plan_evidence || {};
  const bs = providerStatusPill(Boolean(br.disabled), true);
  const bStatus = $("#bufferGuardStatus"); if (bStatus) { bStatus.textContent = bs.text; bStatus.className = `status-pill ${bs.cls}`; }
  const account = snap.account || {}, orgs = snap.organizations || [], channels = snap.channels || [];
  const latest = buffer.last_failure;
  $("#bufferGuardBody").innerHTML = `<div class="provider-facts">
    <span><small>Buffer token</small><strong>${esc(bi.token_masked || "not configured")}</strong><em>Fingerprint ${esc(bi.token_fingerprint || "—")}</em></span>
    <span><small>Connected account</small><strong>${esc(account.name || "Run connection test")}</strong><em>${esc(orgs[0]?.name || "Organization not verified")}</em></span>
    <span><small>Buffer plan evidence</small><strong>${esc(plan.detected || "Not yet inferred")}</strong><em>${plan.expected ? `Expected ${esc(plan.expected)}${plan.matches_expected === false ? " · MISMATCH" : plan.matches_expected === true ? " · matches" : ""}` : esc(plan.note || "Plan field is not exposed by Account")}</em></span>
    <span><small>Channels recognized</small><strong>${aiFmtNumber(channels.length || snap.channel_count || buffer.configured_channels?.length || 0)}</strong><em>${channels.slice(0,5).map(ch => ch.displayName || ch.name || ch.service).filter(Boolean).map(esc).join(" · ") || "Configured IDs only"}</em></span>
    <span><small>API calls</small><strong>${aiFmtNumber(bc.today)} today · ${aiFmtNumber(bc.month)} month</strong><em>${aiFmtNumber(bc.success_today)} success · ${aiFmtNumber(bc.fail_today)} failed · ${aiFmtNumber(bc.retries_today)} retries</em></span>
    <span><small>Official Buffer remaining</small><strong>${rate["15m"] ?? "—"}${rate["15m_quota"] ? `/${rate["15m_quota"]}` : ""} /15m · ${rate["24h"] ?? "—"}${rate["24h_quota"] ? `/${rate["24h_quota"]}` : ""} /24h</strong><em>${rate["30d"] ?? "—"}${rate["30d_quota"] ? `/${rate["30d_quota"]}` : ""} /30d; Samuga safety cap ${aiFmtNumber(bl.requests_day)}/day</em></span>
    <span><small>Duplicate guard</small><strong>${aiFmtNumber(bc.duplicates_prevented_today)} prevented today</strong><em>${latest ? `${esc(latest.error_class || "unknown_error")} · HTTP ${Number(latest.http_status || 0)} · ${esc(latest.message || "")}` : "No recorded failure"}</em></span>
  </div><div class="provider-mini-breakdown"><strong>Buffer request multiplication</strong>${(buffer.by_endpoint || []).slice(0,5).map(item => `<span>${esc(item.endpoint)}<b>${aiFmtNumber(item.requests)} calls · ${aiFmtNumber(item.fail)} failed</b></span>`).join("") || `<span>No requests recorded yet.</span>`}${(buffer.by_channel || []).slice(0,5).map(item => `<span>${esc(item.channel)}<b>${aiFmtNumber(item.requests)} calls · ${aiFmtNumber(item.success)} success</b></span>`).join("")}</div>${(buffer.retryable_publications || []).length ? `<div class="provider-retry-list"><strong>Retryable or delivery-unknown publications</strong>${buffer.retryable_publications.slice(0,8).map(item => `<button type="button" class="secondary compact" data-buffer-retry="${attr(item.idempotency_key)}">${esc(item.channel || item.network || "Channel")} · ${esc(item.error_class || item.status)}</button>`).join("")}</div>` : ""}`;
  $$('[data-buffer-retry]', $("#bufferGuardBody")).forEach(button => button.addEventListener("click", () => providerAction("buffer", "retry", {idempotency_key:button.dataset.bufferRetry})));
  const toggleBuffer = $("#toggleBufferBtn"); if (toggleBuffer) toggleBuffer.textContent = br.disabled ? "Enable auto Buffer" : "Disable auto Buffer";

  const coverage = data.coverage || {}, editorial = data.samuga_editorial_social_cap || {};
  $("#providerEvidenceText").textContent = coverage.historical_limitation || "Detailed data begins when telemetry was deployed.";
  $("#providerIncidentFacts").innerHTML = `<article><span>Telemetry starts</span><strong>${esc(aiDateTime(coverage.telemetry_started_at))}</strong></article><article><span>Recorded AI requests</span><strong>${aiFmtNumber(coverage.recorded_ai_requests)}</strong></article><article><span>Samuga editorial cap</span><strong>${editorial.enabled ? `${editorial.day} day / ${editorial.night} night` : "Disabled"}</strong></article><article><span>Buffer plan distinction</span><strong>Editorial cap ≠ Buffer limit</strong></article>`;
}

async function providerAction(provider, action, extra={}) {
  try {
    const result = await api("/api/admin/provider-diagnostics/action", {method:"POST", body:JSON.stringify({provider, action, ...extra})});
    toast(`${provider === "gemini" ? "Gemini" : "Buffer"} action completed`);
    await loadAIUsage(false);
    return result;
  } catch (error) { toast(error.message, true); return null; }
}

async function toggleGeminiGuard() {
  const disabled = Boolean(providerDiagnostics?.gemini?.runtime?.disabled);
  if (!disabled && !confirm("Disable Gemini immediately? Claude/DeepSeek and manual operations remain available.")) return;
  await providerAction("gemini", disabled ? "enable" : "disable", {reason:"manual_admin_dashboard"});
}
async function toggleBufferGuard() {
  const disabled = Boolean(providerDiagnostics?.buffer?.runtime?.disabled);
  if (!disabled && !confirm("Disable automatic Buffer publishing? Telegram and manual newsroom work remain available.")) return;
  await providerAction("buffer", disabled ? "enable" : "disable", {reason:"manual_admin_dashboard"});
}
async function testBufferPost() {
  const confirmation = prompt("This creates a real test post in the Buffer queue, not an immediate public post. Type BUFFER_TEST_POST to continue.");
  if (confirmation !== "BUFFER_TEST_POST") return;
  await providerAction("buffer", "test_post", {confirmation});
}

function renderAIUsageSummary(data) {
  const today = data.today || {};
  $("#aiTodayRequests").textContent = aiFmtNumber(today.requests);
  $("#aiTodayTokens").textContent = aiFmtNumber(today.tokens);
  $("#aiTodayCost").textContent = aiFmtCost(today.cost);
  $("#aiTodayErrors").textContent = aiFmtNumber(today.errors);
  $("#aiTodayBlocked").textContent = aiFmtNumber(today.blocked);
  $("#aiUsageGeneratedAt").textContent = `Updated ${aiDateTime(data.generated_at)} · ${Number(data.days || 30)}-day analysis range`;

  const todayMap = new Map((today.providers || []).map(item => [item.provider, item]));
  $("#aiTodayProviders").innerHTML = AI_PROVIDER_ORDER.map(provider => {
    const item = todayMap.get(provider) || {requests:0,tokens:0,cost:0};
    return `<article><span>${esc(provider)}</span><strong>${aiFmtNumber(item.requests)} requests</strong><small>${aiFmtNumber(item.tokens)} tokens · ${aiFmtCost(item.cost)}</small></article>`;
  }).join("");

  const alerts = data.alerts || [];
  $("#aiAlertCount").textContent = `${alerts.length} open`;
  $("#aiAlertCount").className = `status-pill ${alerts.some(item => item.severity === "critical") ? "failed" : alerts.length ? "review" : "posted"}`;
  $("#aiUsageAlertBadge").hidden = !alerts.length;
  $("#aiUsageAlertBadge").textContent = alerts.length > 99 ? "99+" : String(alerts.length);
  $("#aiUsageAlerts").innerHTML = alerts.length ? alerts.map(alert => `<article class="ai-alert ${alert.severity === "critical" ? "critical" : "warning"}">
    <div><span>${esc(String(alert.severity || "warning").toUpperCase())} · ${esc(alert.type || "diagnostic")}</span><strong>${esc(alert.title || "AI usage alert")}</strong><p>${esc(alert.detail || "")}</p><small>${[alert.provider,alert.feature,aiDateTime(alert.created_at)].filter(Boolean).map(esc).join(" · ")}</small></div>
    ${ADMIN_ROLES.has(user?.role) ? `<button class="secondary compact" type="button" data-resolve-ai-alert="${Number(alert.id)}">Resolve</button>` : ""}
  </article>`).join("") : `<div class="empty-panel">No open AI usage alerts.</div>`;
  $$('[data-resolve-ai-alert]', $("#aiUsageAlerts")).forEach(button => button.addEventListener("click", () => resolveAIUsageAlert(Number(button.dataset.resolveAiAlert))));

  const features = data.features || [];
  const maxFeatureCost = Math.max(0.000001, ...features.map(item => Number(item.cost || 0)));
  $("#aiFeatureBreakdown").innerHTML = features.length ? features.map(item => `<article class="ai-feature-row">
    <div><strong>${esc(item.feature || "Other")}</strong><span>${Number(item.percentage || 0).toFixed(1)}% · ${aiFmtNumber(item.requests)} requests · ${aiFmtNumber(item.tokens)} tokens</span></div>
    <div class="ai-feature-cost"><strong>${aiFmtCost(item.cost)}</strong><small>${aiFmtNumber(item.errors)} errors</small></div>
    <i><b style="width:${aiBarWidth(item.cost, maxFeatureCost)}%"></b></i>
  </article>`).join("") : `<div class="empty-panel">No AI feature usage in this range.</div>`;

  const providerMap = new Map((data.providers || []).map(item => [item.provider, item]));
  $("#aiProviderBreakdown").innerHTML = AI_PROVIDER_ORDER.map(provider => {
    const item = providerMap.get(provider) || {requests:0,tokens:0,cost:0,errors:0,average_response_ms:0,cached_tokens:0};
    return `<article class="ai-provider-card" data-provider="${attr(provider.toLowerCase())}"><div class="ai-provider-card-head"><span>${esc(provider)}</span><strong>${aiFmtCost(item.cost)}</strong></div><div class="ai-provider-metrics"><span><b>${aiFmtNumber(item.requests)}</b>Requests</span><span><b>${aiFmtNumber(item.tokens)}</b>Tokens</span><span><b>${aiFmtNumber(item.cached_tokens)}</b>Cached</span><span><b>${aiFmtNumber(item.errors)}</b>Errors</span><span><b>${aiFmtDuration(item.average_response_ms)}</b>Avg response</span></div></article>`;
  }).join("");

  const errors = data.errors || [];
  const errorGroups = ["429","500","Timeout","Authentication","Invalid Request","Unknown"];
  const errorMap = new Map(errors.map(item => [item.group, item]));
  $("#aiErrorBreakdown").innerHTML = errorGroups.map(group => {
    const item = errorMap.get(group) || {count:0,last_at:null};
    return `<article class="ai-error-card ${Number(item.count) ? "has-error" : ""}"><span>${esc(group)}</span><strong>${aiFmtNumber(item.count)}</strong><small>${item.last_at ? `Last ${aiDateTime(item.last_at)}` : "No errors"}</small></article>`;
  }).join("");

  renderAIDailyGraphs(data.daily || []);
  renderAITopRequests(data.top_expensive || []);

  const monthlyMap = new Map((data.monthly || []).map(item => [item.provider,item]));
  $("#aiMonthlySummary").innerHTML = AI_PROVIDER_ORDER.map(provider => {
    const item = monthlyMap.get(provider) || {requests:0,tokens:0,cost:0,errors:0};
    return `<article><div><strong>${esc(provider)}</strong><span>${aiFmtNumber(item.requests)} requests · ${aiFmtNumber(item.tokens)} tokens</span></div><div><strong>${aiFmtCost(item.cost)}</strong><small>${aiFmtNumber(item.errors)} errors</small></div></article>`;
  }).join("");

  const options = data.options || {};
  setSelectOptions($("#aiFilterProvider"), options.providers || AI_PROVIDER_ORDER, "All providers");
  setSelectOptions($("#aiFilterFeature"), options.features || [], "All features");
  setSelectOptions($("#aiFilterModel"), options.models || [], "All models");
}

function renderAIDailyGraphs(daily) {
  const container = $("#aiDailyGraphs");
  if (!container) return;
  if (!daily.length) { container.innerHTML = `<div class="empty-panel">Daily usage appears after the first logged AI request.</div>`; return; }
  const series = [
    {key:"requests",label:"Requests",format:aiFmtNumber},
    {key:"tokens",label:"Tokens",format:aiFmtNumber},
    {key:"cost",label:"Estimated cost",format:aiFmtCost},
    {key:"errors",label:"Errors",format:aiFmtNumber},
  ];
  container.innerHTML = series.map(metric => {
    const max = Math.max(0.000001, ...daily.map(item => Number(item[metric.key] || 0)));
    return `<article class="ai-chart"><div class="ai-chart-head"><strong>${metric.label}</strong><span>${metric.format(daily.reduce((sum,item) => sum + Number(item[metric.key] || 0),0))}</span></div><div class="ai-chart-bars">${daily.map(item => {
      const value = Number(item[metric.key] || 0), height = value ? Math.max(4,value/max*100) : 0;
      const date = new Date(`${item.day}T00:00:00`);
      const label = Number.isNaN(date.getTime()) ? String(item.day).slice(5) : date.toLocaleDateString([], {month:"short",day:"numeric"});
      return `<div title="${attr(item.day)} · ${attr(metric.format(value))}"><i style="height:${height}%"></i><span>${esc(label)}</span></div>`;
    }).join("")}</div></article>`;
  }).join("");
}

function renderAITopRequests(rows) {
  const container = $("#aiTopExpensive");
  if (!container) return;
  if (!rows.length) { container.innerHTML = `<div class="empty-panel">No AI requests in this range.</div>`; return; }
  container.innerHTML = `<div class="ai-table-row ai-table-head"><span>Feature</span><span>Provider / model</span><span>Tokens</span><span>Cost</span><span>Duration</span><span>Time</span></div>` + rows.map(item => `<div class="ai-table-row">
    <span><strong>${esc(item.feature)}</strong><small>${esc(item.article_title || item.request_id || "No article context")}</small></span>
    <span><strong>${esc(item.provider)}</strong><small title="${attr(item.model)}">${esc(aiShortModel(item.model))}</small></span>
    <span>${aiFmtNumber(item.total_tokens)}<small>${aiFmtNumber(item.cached_tokens)} cached</small></span>
    <span>${aiFmtCost(item.estimated_cost_usd)}</span><span>${aiFmtDuration(item.duration_ms)}</span><span>${esc(aiDateTime(item.created_at))}</span>
  </div>`).join("");
}

function aiUsageFilterParams() {
  const params = new URLSearchParams();
  const values = {
    provider:$("#aiFilterProvider")?.value || "", feature:$("#aiFilterFeature")?.value || "",
    model:$("#aiFilterModel")?.value || "", status:$("#aiFilterStatus")?.value || "",
    date_from:$("#aiFilterFrom")?.value || "", date_to:$("#aiFilterTo")?.value || "",
  };
  Object.entries(values).forEach(([key,value]) => { if (value) params.set(key,value); });
  return params;
}

async function loadAIUsageRequests(reset = true) {
  if (aiUsageStreamLoading) return;
  aiUsageStreamLoading = true;
  if (reset) aiUsageStreamOffset = 0;
  const params = aiUsageFilterParams();
  params.set("limit", "75"); params.set("offset", String(aiUsageStreamOffset));
  try {
    const data = await api(`/api/admin/ai-usage/requests?${params.toString()}`);
    aiUsageStreamTotal = Number(data.total || 0);
    renderAIUsageRequests(data.requests || [], reset);
    aiUsageStreamOffset += (data.requests || []).length;
    $("#aiLiveCount").textContent = `${aiFmtNumber(aiUsageStreamTotal)} requests`;
    $("#aiLoadMoreBtn").hidden = aiUsageStreamOffset >= aiUsageStreamTotal;
  } catch (error) {
    if (reset) $("#aiLiveRequests").innerHTML = `<div class="empty-panel error">${esc(error.message)}</div>`;
    toast(error.message, true);
  } finally { aiUsageStreamLoading = false; }
}

function renderAIUsageRequests(rows, reset) {
  const container = $("#aiLiveRequests"); if (!container) return;
  const head = `<div class="ai-live-row ai-live-head"><span>Time / status</span><span>Provider / feature</span><span>Model</span><span>Tokens / cost</span><span>Duration</span><span>Context</span></div>`;
  const body = rows.map(item => {
    const context = item.article_title || item.source_url || item.article_id || "No article context";
    const error = item.status === "error" ? (item.error_code || item.error_message || "Unknown error") : item.status === "blocked" ? (item.error_message || "Blocked") : "";
    const safeSourceUrl = aiSafeUrl(item.source_url);
    const contextHtml = safeSourceUrl ? `<a href="${attr(safeSourceUrl)}" target="_blank" rel="noopener" title="${attr(safeSourceUrl)}">${esc(context)}</a>` : `<strong title="${attr(context)}">${esc(context)}</strong>`;
    return `<div class="ai-live-row">
      <span><strong>${esc(aiDateTime(item.created_at))}</strong><small class="status-pill ${aiStatusClass(item.status)}">${esc(item.status)}</small></span>
      <span><strong>${esc(item.provider)}</strong><small>${esc(item.feature)} · retry ${aiFmtNumber(item.retry_count)}</small></span>
      <span title="${attr(item.model)}">${esc(aiShortModel(item.model))}<small>${item.http_status ? `HTTP ${Number(item.http_status)}` : "No HTTP status"}</small></span>
      <span><strong>${aiFmtNumber(item.total_tokens)}</strong><small>${aiFmtNumber(item.input_tokens)} in · ${aiFmtNumber(item.output_tokens)} out · ${aiFmtNumber(item.cached_tokens)} cached<br>${aiFmtCost(item.estimated_cost_usd)}</small></span>
      <span>${aiFmtDuration(item.duration_ms)}${error ? `<small class="ai-error-text">${esc(error)}</small>` : ""}</span>
      <span>${contextHtml}<small>${esc(item.request_id || "")}</small></span>
    </div>`;
  }).join("");
  if (reset) container.innerHTML = rows.length ? head + body : `<div class="empty-panel">No AI requests match these filters.</div>`;
  else if (rows.length) container.insertAdjacentHTML("beforeend", body);
}

async function resolveAIUsageAlert(id) {
  if (!id) return;
  try {
    await api("/api/admin/ai-usage/alerts/resolve", {method:"POST", body:JSON.stringify({id})});
    toast("AI usage alert resolved");
    loadAIUsage(false);
  } catch (error) { toast(error.message, true); }
}

function clearAIUsageFilters() {
  ["aiFilterProvider","aiFilterFeature","aiFilterModel","aiFilterStatus","aiFilterFrom","aiFilterTo"].forEach(id => { const el = $(`#${id}`); if (el) el.value = ""; });
  loadAIUsageRequests(true);
}
...
function wireStaticEvents() {
  ...
  $("#refreshContentLabBtn")?.addEventListener("click", () => requestContentLabRefresh(true));
  ...
  $("#deleteAuthorForm")?.addEventListener("submit", confirmDeleteAuthor);
  $("#socialCardForm")?.addEventListener("submit", createSocialCard);
  $("#socialCardImageUpload")?.addEventListener("change", event => uploadSocialCardImage(event.target.files?.[0]));
  $("#socialCardImageUrl")?.addEventListener("input", renderSocialCardPhotoPreview);
  $("#removeSocialCardImageBtn")?.addEventListener("click", removeSocialCardImage);
  $("#resetSocialCardBtn")?.addEventListener("click", resetSocialCardCreator);
  $("#refreshSocialCardsBtn")?.addEventListener("click", loadSocialCards);
  $$('[data-social-card-post]').forEach(button => button.addEventListener("click", () => postSocialCard(button.dataset.socialCardPost)));
  ...
  $("#testBufferPostBtn")?.addEventListener("click", testBufferPost);
  $("#resetBufferCooldownBtn")?.addEventListener("click", () => providerAction("buffer", "reset_cooldown"));
  $("#aiUsageDays")?.addEventListener("change", () => loadAIUsage(true));
...
function showApp() {
  ...
  if (PUBLISH_ROLES.has(user?.role) && requested !== "aiusage") loadAIUsage(false);
}
...
function resetEditor() {
  ...
  if ($("#articleAuthor") && user?.author_id) $("#articleAuthor").value = user.author_id;
...
async function saveArticle(forceDraft) {
  ...
  try {
    ...
    const publishedNow = payload.status === "posted" || ["posted","published","social_posted"].includes(data.article.status);
    if (publishedNow) {
      resetEditor();
      openView("editor");
      $("#saveState").textContent = "Ready for a new article";
      setTimeout(() => $("#articleTitle")?.focus(), 80);
      toast("Published — the editor is ready for your next article");
    }
    await loadDashboard();
...
async function openMediaPicker(target) {
  mediaPickerTarget = ["inline","socialcard"].includes(target) ? target : "cover";
...
function chooseMedia(item) {
  if (!item || item.status === "failed" || ["pending","processing"].includes(item.status)) return;
  if (mediaPickerTarget === "cover") useMediaAsCover(item);
  else if (mediaPickerTarget === "socialcard") {
    if (item.type !== "image") { toast("Choose an image for a social-card background.", true); return; }
    $("#socialCardImageUrl").value = item.url; renderSocialCardPhotoPreview();
  } else addMediaInline(item);
...
function startContentLabRefreshTimer() {
  clearInterval(contentLabRefreshTimer);
  contentLabRefreshTimer = null;
  if (!$("#view-contentlab")?.classList.contains("active") || contentLabEditingKey || contentLabEditorLocks.size) return;
  contentLabRefreshTimer = setInterval(() => {
    if (document.visibilityState === "visible" && !contentLabEditingKey && !contentLabEditorLocks.size && !contentLabInteractionActive()) requestContentLabRefresh(false);
  }, 30000);
}
function pauseContentLabRefresh(key) {
  contentLabEditingKey = String(key || contentLabEditingKey || "");
  if (contentLabEditingKey) contentLabEditorLocks.add(contentLabEditingKey);
  clearInterval(contentLabRefreshTimer);
  contentLabRefreshTimer = null;
  contentLabPendingRefresh = false;
  if (contentLabRefreshController) {
    try { contentLabRefreshController.abort(); } catch {}
    contentLabRefreshController = null;
  }
  const state = $("#contentLabConnection");
  if (state) state.textContent = "Editing — live refresh is locked off. Your text will stay open until you close or publish it.";
}
function resumeContentLabRefresh(key) {
  if (key) contentLabEditorLocks.delete(String(key));
  if (!key || String(key) === contentLabEditingKey) contentLabEditingKey = "";
  if (!contentLabEditorLocks.size && !contentLabEditingKey) {
    const state = $("#contentLabConnection");
    if (state) state.textContent = "Editor closed — live queue refresh will resume shortly.";
    window.setTimeout(() => {
      if (!contentLabEditingKey && $("#view-contentlab")?.classList.contains("active")) {
        startContentLabRefreshTimer();
        requestContentLabRefresh(false);
      }
    }, 900);
  }
}
function contentLabInteractionActive() {
  if (!$("#view-contentlab")?.classList.contains("active")) return false;
  const active = document.activeElement;
  return Boolean(
    contentLabEditingKey ||
    contentLabEditorLocks.size ||
    active?.matches?.(".lab-edit-headline, .lab-edit-paragraph") ||
    $("#contentLabList .lab-edit-box.open") ||
    $("#contentLabList .lab-working") ||
    $('#contentLabList [data-dirty="true"]')
  );
}
function contentLabSignature(items, history) {
  return JSON.stringify({
    items:(items || []).map(item => [item.key,item.title,item.summary,item.caption,item.dv_text,item.created_at,item.manual_only,item.review_type,item.review_details]),
    history:(history || []).map(row => [row.key,row.status,row.action,row.updated_at,row.actioned_at])
  });
}

async function requestContentLabRefresh(force = false) {
  if (!PUBLISH_ROLES.has(user?.role) || contentLabEditingKey) return;
  if (contentLabRefreshInFlight) {
    contentLabPendingRefresh = true;
    return;
  }
  if (contentLabInteractionActive()) {
    contentLabPendingRefresh = true;
    return;
  }
  await loadContentLab({force});
}

async function loadContentLab({force = false} = {}) {
  if (!PUBLISH_ROLES.has(user?.role) || contentLabEditingKey) return;
  if (!force && contentLabInteractionActive()) { contentLabPendingRefresh = true; return; }
  if (contentLabRefreshInFlight) { contentLabPendingRefresh = true; return; }
  contentLabRefreshInFlight = true;
  const controller = new AbortController();
  contentLabRefreshController = controller;
  const state = $("#contentLabConnection");
  try {
    const data = await api(`/api/admin/content-lab?build=${SAMUGA_BUILD}`, {signal:controller.signal});
    if (controller.signal.aborted || contentLabEditingKey || (!force && contentLabInteractionActive())) { contentLabPendingRefresh = false; return; }
    const items = data.items || [], counts = data.counts || {};
    if (state) state.textContent = String(data.sync_mode || "").startsWith("shared_approval_queue")
      ? `Connected — dashboard and Telegram are using the same live approval queue${data.telegram_linked === false ? " (Telegram settings need attention)" : ""}. Auto-refresh pauses while you edit.`
    ...
    const signature = contentLabSignature(items, history);
    if (force || signature !== contentLabLastSignature) {
      contentLabLastSignature = signature;
      renderContentLab(items);
      renderContentLabHistory(history);
      await loadContentLabImages(items);
    }
  } catch (error) {
    if (error?.name !== "AbortError") { showContentLabError(error); toast(error.message, true); }
  }
  finally {
    if (contentLabRefreshController === controller) contentLabRefreshController = null;
    contentLabRefreshInFlight = false;
    if (contentLabPendingRefresh && !contentLabInteractionActive()) {
      contentLabPendingRefresh = false;
      setTimeout(() => requestContentLabRefresh(false), 50);
    }
  }
}
function renderContentLab(items) {
  ...
  if (!items.length) { container.innerHTML = `<div class="empty-panel">Content Lab is clear. New Telegram review cards will appear here automatically.</div>`; startContentLabRefreshTimer(); return; }
  container.innerHTML = items.map(item => {
    const serverHeadline = item.edit_headline || item.title || "";
    const serverParagraph = item.edit_paragraph || item.summary || "";
    const savedDraft = contentLabDrafts.get(item.key) || {};
    const headline = savedDraft.headline ?? serverHeadline;
    const paragraph = savedDraft.paragraph ?? serverParagraph;
    const direction = item.lang === "dv" ? "rtl" : "ltr";
    const isWeatherReview = item.review_type === "weather_alert";
    const reviewDetails = String(item.review_details || "").trim();
    const showReviewDetails = reviewDetails && reviewDetails !== paragraph && reviewDetails !== `${headline}\n\n${paragraph}`.trim();
    return `<article class="lab-card ${item.breaking ? "breaking" : ""} ${isWeatherReview ? "weather-review" : ""}" data-lab-key="${attr(item.key)}" dir="${direction}">
      <div class="lab-card-media">${item.has_card ? `<span class="no-card">Loading full card…</span>` : `<span class="no-card">Text preview</span>`}</div>
      <div class="lab-card-body">
        <div class="lab-card-kicker"><div><span class="lab-key">${esc(String(item.key || "").toUpperCase())}</span><span class="lab-category">${esc(item.category || "LOCAL")}</span><span class="lab-language">${isWeatherReview ? `Weather review ${Math.round(Number(item.confidence || 0) * 100)}%` : (item.lang === "dv" ? "Dhivehi" : "English")}</span></div><span class="lab-age">${esc(relativeAdminTime(item.created_at))}</span></div>
        <h3 class="lab-title">${esc(headline || "Untitled")}</h3>
        ${paragraph ? `<div class="lab-copy">${esc(paragraph)}</div>` : ""}
        ${showReviewDetails ? `<div class="lab-review-caption"><strong>Review details</strong><p>${esc(reviewDetails)}</p></div>` : ""}
        <div class="lab-actions">
          <button data-lab-action="post_tg" type="button">${isWeatherReview ? "✅ Confirm alert" : "📣 Post to Telegram"}</button><button data-lab-action="post_soc" type="button">${isWeatherReview ? "✅ Confirm alert" : "📱 Post to Social"}</button>
          <button class="lab-all" data-lab-action="post_all" type="button">${isWeatherReview ? "✅ Confirm + publish" : "🌐 Post to All"}</button><button data-lab-edit type="button">✏️ Edit</button><button class="lab-reject" data-lab-action="reject" type="button">❌ Reject</button>
        </div>
        <div class="lab-edit-box" aria-hidden="true">
          <div class="lab-edit-fields">
            <label><span>Card headline</span><input class="lab-edit-headline" dir="${direction}" maxlength="240" value="${attr(headline)}" placeholder="Headline shown on the generated card"></label>
            <label><span>Card paragraph</span><textarea class="lab-edit-paragraph" dir="${direction}" maxlength="1200" placeholder="Short paragraph shown below the headline">${esc(paragraph)}</textarea></label>
          </div>
          <p class="lab-edit-help">Only the public headline and paragraph are editable here. Source links and internal review notes are never inserted into these fields.</p>
          <div class="lab-edit-destinations"><button data-lab-edit-action="post_tg" type="button">Save + Telegram</button><button data-lab-edit-action="post_soc" type="button">Save + Social</button><button data-lab-edit-action="post_all" type="button">Save + All</button></div>
        </div>
      </div></article>`;
  }).join("");
  ...
  $$('[data-lab-edit]', container).forEach(button => button.addEventListener("click", () => {
    const card = button.closest("[data-lab-key]");
    const editBox = card?.querySelector(".lab-edit-box");
    if (!card || !editBox) return;
    const opening = !editBox.classList.contains("open");
    editBox.classList.toggle("open", opening);
    editBox.setAttribute("aria-hidden", opening ? "false" : "true");
    card.classList.toggle("editing", opening);
    button.textContent = opening ? "✕ Close edit" : "✏️ Edit";
    if (opening) {
      pauseContentLabRefresh(card.dataset.labKey);
      requestAnimationFrame(() => card.querySelector(".lab-edit-headline")?.focus());
    } else {
      card.querySelectorAll('[data-dirty="true"]').forEach(field => { delete field.dataset.dirty; });
      resumeContentLabRefresh(card.dataset.labKey);
    }
  }));
  $$('.lab-edit-headline, .lab-edit-paragraph', container).forEach(field => field.addEventListener("input", () => {
    const card = field.closest("[data-lab-key]");
    if (!card) return;
    const draft = {
      headline: card.querySelector(".lab-edit-headline")?.value || "",
      paragraph: card.querySelector(".lab-edit-paragraph")?.value || ""
    };
    contentLabDrafts.set(card.dataset.labKey, draft);
  ...
  $$('[data-lab-edit-action]', container).forEach(button => button.addEventListener("click", () => {
    const card = button.closest("[data-lab-key]");
    const corrected = {
      headline: card.querySelector(".lab-edit-headline")?.value.trim() || "",
      paragraph: card.querySelector(".lab-edit-paragraph")?.value.trim() || ""
    };
...
async function loadContentLabImages(items) {
  await Promise.all(items.filter(item => item.has_card).map(async item => {
    try {
      ...
      if (media) media.innerHTML = `<button type="button" class="lab-card-image-button" title="Open full-size card"><img src="${url}" alt="Full Content Lab card for ${attr(item.title || item.key)}"></button><span class="lab-preview-note">Full card preview · click to enlarge</span>`;
      media?.querySelector(".lab-card-image-button")?.addEventListener("click", () => window.open(url, "_blank", "noopener,noreferrer"));
    } catch {}
  }));
}
async function runContentLabAction(card, action, corrected = null) {
  if (!card) return;
  const key = card.dataset.labKey;
  const weatherReview = card.querySelector(".lab-language")?.textContent?.includes("Weather review");
  const label = weatherReview && action !== "reject" ? "Confirm that this is a genuine official MMS alert and publish the weather alert?" : ({post_tg:"Post this card to Telegram? The website will also be published.",post_soc:"Post this card to social platforms? The website will also be published.",post_all:"Post this card everywhere?",reject:"Reject and remove this card?"}[action] || "Continue?");
  if (!confirm(label)) return;
  card.classList.add("lab-working");
  clearInterval(contentLabRefreshTimer); contentLabRefreshTimer = null;
  contentLabEditorLocks.delete(String(key));
  if (contentLabEditingKey === String(key)) contentLabEditingKey = "";
  contentLabPendingRefresh = false;
  try {
    const payload = {key, action};
    if (corrected && typeof corrected === "object") {
      payload.corrected_title = corrected.headline || "";
      payload.corrected_paragraph = corrected.paragraph || "";
    } else if (corrected) {
      payload.corrected = corrected;
    }
    const data = await api("/api/admin/content-lab/action", {method:"POST",body:JSON.stringify(payload)});
    contentLabDrafts.delete(key); contentLabEditorLocks.delete(String(key)); toast(data.message || `${String(key).toUpperCase()} action accepted`); await loadContentLab({force:true}); startContentLabRefreshTimer();
...
function renderAuthorProfiles(authors) {
  ...
  container.innerHTML = authors.map(author => {
    ...
    return `<article class="author-card ${author.active === false ? "inactive" : ""}">
      ...
      <div class="author-card-actions">${canEdit ? `<button class="secondary compact" data-edit-author="${attr(author.author_id)}">Edit profile</button>` : ""}${user?.role === "super_admin" && author.author_id !== "samuga_ai" ? `<button class="danger-button compact" data-delete-author="${attr(author.author_id)}">Delete</button>` : ""}</div>
    </article>`;
  }).join("");
  ...
  $$('[data-delete-author]', container).forEach(button => button.addEventListener("click", () => openDeleteAuthorDialog(authors.find(author => author.author_id === button.dataset.deleteAuthor))));
}
...
function openDeleteAuthorDialog(author) {
  if (!author || user?.role !== "super_admin") return;
  $("#deleteAuthorError").textContent = "";
  $("#deleteAuthorId").value = author.author_id;
  const dependencies = Number(author.article_count || 0) + Number(author.linked_users || 0);
  $("#deleteAuthorMessage").textContent = dependencies
    ? `${author.name} has ${Number(author.article_count || 0)} article(s) and ${Number(author.linked_users || 0)} linked login(s). Choose the author that should receive them.`
    : `${author.name} is unused and can be permanently deleted.`;
  const targets = currentAuthors.filter(item => item.author_id !== author.author_id && item.active !== false);
  $("#deleteAuthorTargetRow").hidden = !dependencies;
  $("#deleteAuthorTarget").innerHTML = `<option value="">Choose replacement author</option>` + targets.map(item => `<option value="${attr(item.author_id)}">${esc(item.name)} — ${esc(item.role || "Reporter")}</option>`).join("");
  $("#deleteAuthorDialog").showModal();
}

async function confirmDeleteAuthor(event) {
  event.preventDefault(); event.stopPropagation();
  const authorId = $("#deleteAuthorId").value;
  const reassignTo = $("#deleteAuthorTargetRow").hidden ? null : $("#deleteAuthorTarget").value;
  $("#deleteAuthorError").textContent = "";
  if (!$("#deleteAuthorTargetRow").hidden && !reassignTo) { $("#deleteAuthorError").textContent = "Choose a replacement author first."; return; }
  const button = $("#confirmDeleteAuthorBtn"); button.disabled = true; button.textContent = "Deleting…";
  try {
    const data = await api("/api/admin/authors/delete", {method:"POST", body:JSON.stringify({author_id:authorId,reassign_to:reassignTo})});
    $("#deleteAuthorDialog").close(); toast(data.message || "Author deleted");
    const me = await api("/api/admin/me"); user = me.user;
    await loadAuthorProfiles(); await loadAuthors(true);
  } catch (error) { $("#deleteAuthorError").textContent = error.message; }
  finally { button.disabled = false; button.textContent = "Delete author"; }
}
...
function renderUsers(users) {
  ...
  container.innerHTML = `<div class="table-row header user-table-row"><span>Name</span><span>Email</span><span>Role</span><span>Author</span><span></span></div>` + users.map(account => {
    const linked = authorById.get(account.author_id);
    const role = String(account.role || "").replaceAll("_", " ");
    return `<div class="table-row user-table-row"><div class="article-cell"><strong title="${attr(account.name)}">${esc(account.name)}</strong><span>Last login: ${esc(shortDateTime(account.last_login))}</span></div><span class="user-email" title="${attr(account.email)}">${esc(account.email)}</span><span class="user-role" title="${attr(role)}">${esc(role)}</span><span class="user-author" title="${attr(linked?.name || "Not linked")}">${esc(linked?.name || "Not linked")}</span><div class="row-actions"><button data-edit-user="${account.id}">Edit</button></div></div>`;
  }).join("");
