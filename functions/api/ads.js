...
const HEADERS = {"content-type":"application/json;charset=utf-8","cache-control":"public,max-age=30,s-maxage=60",...SECURITY_HEADERS};
export async function onRequest({request, env}) {
  ...
  try {
    const upstream = await fetchWithTimeout(`${backendBase(env)}/api/ads${url.search}`, {headers:{accept:"application/json"},cf:{cacheTtl:30,cacheEverything:true}}, Number(env?.UPSTREAM_TIMEOUT_MS || 9000));
