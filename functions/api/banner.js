...
export async function onRequest({request, env}) {
  ...
  try {
    const upstream = await fetchWithTimeout(`${backendBase(env)}/api/banner${url.search}`, {headers:{accept:"application/json"},cf:{cacheTtl:20,cacheEverything:true}}, Number(env?.UPSTREAM_TIMEOUT_MS || 9000));
