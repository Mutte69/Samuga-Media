...
(() => {
  ...
  const SESSION_KEY = "samuga-ai-floating-history-v2";
  ...
  function applyFooter(s) {
    ...
    const links = {
      "/about": s.about_url, "/contact": s.contact_url, "/advertising": s.advertising_url,
      "/privacy-policy": s.privacy_url, "/terms": s.terms_url,
      "/editorial-policy": s.editorial_policy_url, "/corrections-policy": s.corrections_policy_url
    };
    $$(".v3-footer-links a,.v3-drawer-links a").forEach(a => {
      const path = new URL(a.href, location.origin).pathname.replace(/\.html$/, "");
      const replacement = links[path];
      if (safeUrl(replacement)) a.href = safeUrl(replacement);
    });
  }
  function applyAI(s) {
    const enabled = s.ai_enabled === undefined || s.ai_enabled === null || s.ai_enabled === "" ? true : asBool(s.ai_enabled, true);
    ...
    if (fab) {
      fab.hidden = !showFloating;
      fab.dataset.position = ["bottom-left","bottom-right"].includes(s.ai_button_position) ? s.ai_button_position : "bottom-right";
    }
    if (panel && !enabled) {
    ...
    const first = $("#chatMessages .msg.bot");
    if (first && s.ai_welcome_message && $$("#chatMessages .msg").length === 1) first.textContent = s.ai_welcome_message;
    const input = $("#chatInput");
    if (input && s.ai_placeholder) input.placeholder = s.ai_placeholder;
    document.body.classList.toggle("settings-ai-mobile-hidden", !asBool(s.ai_mobile_enabled, true));
  }
  function applyHomepage(s) {
    const toggle = (selector,key,fallback=true) => $$(selector).forEach(node => { const hidden = !asBool(s[key],fallback); node.hidden = hidden; if (selector === ".lead-section") node.dataset.settingsHidden = String(hidden); });
  ...
  function applySettings(s) {
    ...
    document.documentElement.dataset.samugaSettings = "16.2.1";
    ...
    let savedTheme = null; try { savedTheme = localStorage.getItem("samuga-theme"); } catch {}
    if (!savedTheme && ["light","dark"].includes(s.default_theme)) document.documentElement.dataset.theme=s.default_theme;
