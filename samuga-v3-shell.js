...
(() => {
  ...
  const BUILD = "16.0.0-v3-public-shell";
  ...
  const preferredLang = () => localStorage.getItem("samuga-lang") === "dv" ? "dv" : "en";

  function socialLinkMarkup(key, cssClass = "") {
    const url = safeUrl(socials[key]);
    const disabled = !url;
    return `<a class="v3-social-link ${cssClass}" data-social-key="${key}" href="${disabled ? "#" : url}" ${disabled ? 'aria-disabled="true" tabindex="-1"' : 'target="_blank" rel="noopener"'} aria-label="${SOCIAL_LABELS[key]}" title="${disabled ? `${SOCIAL_LABELS[key]} link is not configured` : SOCIAL_LABELS[key]}">${ICONS[key]}</a>`;
  }
  ...
  function drawerMarkup() {
    ...
    return `
      ...
      <aside class="v3-drawer" id="v3Drawer" role="dialog" aria-modal="true" aria-label="Samuga Media menu" aria-hidden="true" tabindex="-1">
        ...
        <button class="v3-ai-drawer-btn" id="v3AskAi" type="button"><span aria-hidden="true">✦</span><span data-v3-ai-label>Ask Samuga AI</span></button>
        <section class="v3-drawer-section" aria-labelledby="v3FollowHeading">
          ...
          <div class="v3-social-row" data-v3-socials>${SOCIAL_KEYS.map(key => socialLinkMarkup(key)).join("")}</div>
        </section>
  ...
  function addDrawer() {
    ...
    bindCategories();
    bindNewsletterForms();
    q("#v3AskAi")?.addEventListener("click", () => {
      closeDrawer();
      ensureChat();
      window.setTimeout(() => q("#chatFab")?.click(), 80);
    });
  }
  ...
  function bindDrawer() {
    ...
    drawer?.addEventListener("click", event => {
      if (event.target.closest("a[href]") && !event.target.closest('[aria-disabled="true"]')) closeDrawer();
    });
  ...
  function bindCategories() {
    qa("[data-v3-category]").forEach(button => button.addEventListener("click", () => {
      ...
      const homepageButton = q(`.nav-btn[data-cat="${CSS.escape(category)}"]`);
      if (homepageButton) {
  ...
  function updateLanguage(lang = preferredLang()) {
    ...
    if (drawerEl) drawerEl.dir = lang === "dv" ? "rtl" : "ltr";
  }
  ...
  function hydrateSocialLinks() {
    ...
    qa("[data-social-key]").forEach(link => {
      ...
      if (url) {
        ...
        link.title = `${SOCIAL_LABELS[key]} link is not configured`;
      }
    });
  }
  ...
  function ensureChat() {
    if (q("#chatFab") && q("#chatPanel")) return;
  ...
  function bindGenericChat() {
    ...
    genericChatBound = true;
    const open = () => {
      panel.hidden = false; panel.classList.add("open"); panel.setAttribute("aria-hidden", "false"); fab.setAttribute("aria-expanded", "true"); document.body.classList.add("chat-open");
      window.setTimeout(() => input.focus(), 30);
    };
    const hide = () => {
      panel.classList.remove("open"); panel.setAttribute("aria-hidden", "true"); fab.setAttribute("aria-expanded", "false"); document.body.classList.remove("chat-open"); input.blur();
    };
    ...
    form.addEventListener("submit", async event => {
      ...
      if (!message) return;
      addChatMessage(messages, message, "user"); input.value = ""; send.disabled = true;
      const wait = addChatMessage(messages, "Samuga AI is checking…", "bot");
      try {
        const response = await fetch(`${API}/api/chat`, {method: "POST", headers: {"Content-Type": "application/json"}, body: JSON.stringify({message, lang: preferredLang()})});
        const data = await response.json(); wait.remove(); addChatMessage(messages, data.reply || "I could not find an answer right now.", "bot");
      } catch { wait.remove(); addChatMessage(messages, "Connection issue. Please try again.", "bot"); }
  ...
  function bindStandaloneLanguageButtons() {
    qa(".lang-btn").forEach(button => {
      ...
      button.addEventListener("click", () => {
        ...
        localStorage.setItem("samuga-lang", lang);
