...
const SAMUGA_SITE_BUILD="16.2.0";
...
function applySiteSettings(){
  ...
  const enabled=boolSetting(siteSettings.ai_enabled,boolSetting(siteSettings.show_ai_chat,true));
  const showAi=enabled&&boolSetting(siteSettings.ai_floating_enabled,true);
  const fab=$("#chatFab"),panel=$("#chatPanel");
  if(fab)fab.hidden=!showAi;
  if(panel&&!showAi){panel.classList.remove("open");panel.hidden=true;panel.setAttribute("aria-hidden","true");document.body.classList.remove("chat-open")}
...
function setupChat(){
  ...
  if(!fab||!panel||fab.dataset.chatBound==="true")return;
  ...
  const fitMobileViewport=()=>{
    ...
    const panelHeight=keyboardOpen
      ? Math.max(280,viewportHeight-(verticalGap*2))
      : Math.min(520,Math.max(360,Math.round(viewportHeight*0.68)));
  ...
  form?.addEventListener("submit",async event=>{
    ...
    try{
      const response=await apiFetch("/api/chat",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({message,lang:activeLang,surface:"floating"})});
