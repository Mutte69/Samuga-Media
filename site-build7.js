...
function setupChat(){
  ...
  const fitMobileViewport=()=>{
    ...
    const top=Math.round((vv?.offsetTop||0)+8);
    const height=Math.max(300,Math.round((vv?.height||window.innerHeight)-16));
    panel.style.top=`${top}px`;
    panel.style.height=`${height}px`;
    panel.style.bottom="auto";
    requestAnimationFrame(()=>{msgs.scrollTop=msgs.scrollHeight});
  };
  const resetViewport=()=>{panel.style.removeProperty("top");panel.style.removeProperty("height");panel.style.removeProperty("bottom")};
  const open=()=>{
  ...
  const hide=()=>{
    panel.classList.remove("open");panel.setAttribute("aria-hidden","true");fab.setAttribute("aria-expanded","false");document.body.classList.remove("chat-open");resetViewport();
  };
  ...
  input?.addEventListener("focus",()=>setTimeout(fitMobileViewport,80));
  form?.addEventListener("submit",async event=>{
    event.preventDefault();const message=input.value.trim();if(!message)return;
    addMessage(msgs,message,"user");input.value="";send.disabled=true;fitMobileViewport();
    ...
    try{
      ...
      const data=await response.json();wait.remove();addMessage(msgs,data.reply||"I could not find an answer right now.","bot");
    }catch{wait.remove();addMessage(msgs,"Connection issue. Please try again.","bot")}
    finally{send.disabled=false;fitMobileViewport();if(!isMobile())input.focus()}
  });
}
