...
const SAMUGA_SITE_BUILD="16.0.1";
...
async function apiFetch(path,options={}){
  ...
  for(const url of attempts){
    try{
      ...
      if(response.status>=400&&response.status<500&&response.status!==404&&response.status!==408&&response.status!==429)break;
...
function setupLanguage(){
  $$(".lang-btn").forEach(b=>b.addEventListener("click",()=>{activeLang=b.dataset.lang||"en";safeStorage.set("samuga-lang",activeLang);applyLanguage();renderAll()}));
}
...
function setupChat(){
  ...
  form?.addEventListener("submit",async event=>{
    ...
    try{
      const response=await apiFetch("/api/chat",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({message,lang:activeLang})});
