...
# Samuga Cortex is the observe-first journalist thinking engine.
# Optional by design: if the file is missing or has an error, the live bot
# keeps running with the old workflow.
try:
    from samuga_cortex import cortex_decide, format_cortex_admin_block
    CORTEX_AVAILABLE = True
except Exception as _cortex_import_error:
    cortex_decide = None
    format_cortex_admin_block = None
    CORTEX_AVAILABLE = False

# brain_memory records every editor action (approve/reject/edit) so Cortex can
# learn newsroom preferences over time. Optional — bot runs fine without it.
try:
    from brain_memory import record_editor_action as _bm_record, learned_boost_for_story as _bm_boost
    BRAIN_MEMORY_AVAILABLE = True
except Exception:
    _bm_record = None
    _bm_boost  = None
    BRAIN_MEMORY_AVAILABLE = False

def _bm_log(action, item=None, article=None, member="Team", extra=None):
    """Safe wrapper — records one editor action to brain_memory. Never raises."""
    if not BRAIN_MEMORY_AVAILABLE or not _bm_record:
        return
    try:
        src   = (item or article or {})
        score = src.get("_priority") or src.get("_funnel_score") or src.get("score") or 0
        _bm_record(
            human_action    = action,
            article_id      = src.get("article_id") or src.get("id") or "",
            source          = src.get("source",""),
            title           = src.get("title",""),
            summary         = src.get("summary",""),
            language        = src.get("lang", "en"),
            category        = src.get("cat",""),
            story_type      = src.get("_story_type",""),
            score_before    = float(score) if score else None,
            confidence_score= float(src.get("_confidence",0) or 0) or None,
            risk_score      = float(src.get("_risk_score",0) or 0) or None,
            reason          = str(extra or ""),
            created_by      = member,
            posted_platforms= _bm_platforms(action),
            metadata        = {"key": src.get("_queue_key",""), "destination": str(extra or "")},
        )
    except Exception as _bm_err:
        log.debug(f"[BRAIN] record skipped: {_bm_err}")

def _bm_platforms(action):
    """Map action string to list of platforms that were used."""
    mapping = {
        "post_all":      ["telegram","facebook","instagram","x","website"],
        "post_telegram": ["telegram","website"],
        "post_social":   ["facebook","instagram","x","website"],
        "approve":       ["telegram","facebook","instagram","x","website"],
        "reject":        [],
        "edited":        ["telegram","facebook","instagram","x","website"],
        "edit":          [],
        "expired":       [],
        "skip":          [],
    }
    return mapping.get(str(action).lower(), [])

def _bm_action_for(destination):
    """Translate publish_approved_item destination to brain_memory action label."""
    return {
        "telegram": "post_telegram",
        "social":   "post_social",
        "all":      "post_all",
    }.get(str(destination).lower(), "post_all")


def _db_ok():
    """Read db.DB_ENABLED live — avoids the stale import bug.

    bot.py imports DB_ENABLED at startup (value = False).
    init_database() later sets db.DB_ENABLED = True on the module object,
    but bot.py's local name is never updated.
    This function always reads the current module attribute, so every
    command handler sees the real connection state.
    """
    try:
        import db as _db_mod
        return bool(_db_mod.DB_ENABLED)
    except Exception:
        return False
...
# English auto-post timers — tiered by score + confidence
# High score (>=140) + high confidence (>=70%): 5 min   → team sees preview, fires fast
# High score (>=140) + high confidence (>=70%):  5 min  → fast-track, preview only
# Everything else:                               15 min  → short review window
# 45-min default eliminated — all EN cards auto-post within 15 min max
ENGLISH_AUTOPOST_SECONDS        = 900    # Default: 15 min (was 45)
ENGLISH_AUTOPOST_HIGH_SECONDS   = 300    # High score + confident: 5 min
ENGLISH_AUTOPOST_MED_SECONDS    = 900    # Medium confidence: 15 min (same as default)
ENGLISH_AUTOPOST_HIGH_SCORE     = 140    # Score threshold for fast-track
ENGLISH_AUTOPOST_HIGH_CONF      = 70     # Confidence % threshold for 5-min
ENGLISH_AUTOPOST_MED_CONF       = 0      # All non-high articles use 15 min (effectively always true)
...
def expire_old_approvals():
    ...
    # ── English auto-post: tiered by score + confidence ──────────────────────
    # High score (>=140) + high confidence (>=70%): 5 min  — fast-track, preview in CL
    # Medium confidence (>=45%):                   15 min  — quick window, then fire
    # Default:                                     45 min  — full review window
    def _en_autopost_seconds(item):
        """
        Two-tier auto-post timing for EN Content Lab cards:
          High score (>=140) + high confidence (>=70%) → 5 min  (preview then fire fast)
          Everything else                              → 15 min (short review window)
        No card waits longer than 15 minutes. Website already has the article.
        """
        score = float(item.get("_priority") or item.get("score") or 0)
        conf  = float(item.get("_confidence") or 0)
        if score >= ENGLISH_AUTOPOST_HIGH_SCORE and conf >= ENGLISH_AUTOPOST_HIGH_CONF:
            return ENGLISH_AUTOPOST_HIGH_SECONDS   # 5 min — high confidence fast-track
        return ENGLISH_AUTOPOST_SECONDS            # 15 min — standard window

    en_due = [k for k, v in approval_queue.items()
              if v.get("lang") == "en"
              and not v.get("is_breaking", False)
              and (now - v["created_at"]).total_seconds() > _en_autopost_seconds(v)]

    for k in en_due:
        item = approval_queue.pop(k)
        title  = item.get("title","")[:50]
        score  = float(item.get("_priority") or 0)
        conf   = float(item.get("_confidence") or 0)
        waited = int((now - item["created_at"]).total_seconds() // 60)

        # Tier label for the alert
        if score >= ENGLISH_AUTOPOST_HIGH_SCORE and conf >= ENGLISH_AUTOPOST_HIGH_CONF:
            tier_label = f"5min fast-track (score={int(score)}, conf={int(conf)}%)"
        else:
            tier_label = f"15min standard (score={int(score)}, conf={int(conf)}%)"

        log.info(f"⏰ EN {k} auto-posting [{tier_label}]: {title}")
        try:
            buf = io.BytesIO(item["card_bytes"])
            queue_for_social(buf, item["caption"],
                key_label=f"{k.upper()} (auto {waited}m)",
            ...
            send_text(CORE_TEAM_CHAT_ID,
                f"⏰ <b>{k.upper()}</b> auto-posted ({waited}min) [{tier_label}]\n"
                f"📰 {item.get('title','')[:80]}",
                thread_id=ALERT_THREAD_ID)
        except Exception as e:
...
PUBLIC_TIPS_THREAD_ID = 13680  # public tip submissions from @SamugaNewsBot DMs

# ── Front Desk config ──────────────────────────────────────────────────────────
PUBLIC_INACTIVITY_MINUTES = 15   # show menu again after this many minutes idle

# Bilingual welcome (shown once per user on /start or first message)
_FD_WELCOME = (
    "ސަމުގާ މީޑިއާއަށް މަރުޙަބާ 👋\n"
    "<b>Welcome to Samuga Media 👋</b>\n\n"
    "ތިޔަބޭފުޅާ ކުރައްވަން ބޭނުންފުޅުވަނީ ކޮން ކަމެއްތޯ؟\n"
    "What would you like to do?\n\n"
    "📰 <b>Latest news</b> — އެންމެ ފަހުގެ ޚަބަރު\n"
    "🚨 <b>Report</b> — ކަމެއް ރިޕޯޓްކުރެއްވުމަށް\n"
    "💬 <b>Ask Samuga AI</b> — ސަމުގާ އޭއައި އާ ސުވާލުކުރައްވާ"
)

# Keyboards built lazily on first use — _make_inline_kb is defined later in this file
_FD_MAIN_KB        = None
_FD_SMALL_KB       = None
_FD_AFTER_REPORT_KB = None

def _fd_kb(name):
    """Return the named Front Desk keyboard, building it on first call."""
    global _FD_MAIN_KB, _FD_SMALL_KB, _FD_AFTER_REPORT_KB
    if _FD_MAIN_KB is None:
        _FD_MAIN_KB = _make_inline_kb([
            [("📰 Latest news", "fd:news"), ("🚨 Report", "fd:report")],
            [("💬 Ask Samuga AI", "fd:chat")],
        ])
        _FD_SMALL_KB = _make_inline_kb([
            [("📰 Latest news", "fd:news"), ("🚨 Report", "fd:report"), ("💬 Ask Samuga AI", "fd:chat")],
        ])
        _FD_AFTER_REPORT_KB = _make_inline_kb([
            [("📰 Get latest news", "fd:news"), ("🚨 Report another", "fd:report")],
            [("💬 Ask Samuga AI", "fd:chat")],
        ])
    _FD_ANON_KB = _make_inline_kb([
        [("🕵️ Stay anonymous", "fd:anon_yes"),
         ("📞 Samuga can contact me", "fd:anon_no")],
    ])
    return {
        "main":         _FD_MAIN_KB,
        "small":        _FD_SMALL_KB,
        "after_report": _FD_AFTER_REPORT_KB,
        "anon":         _FD_ANON_KB,
    }[name]

# ── Front Desk in-memory state ──────────────────────────────────────────────
# Keyed by user_id (int). Survives Railway restart via kv_set/kv_get on change.
_fd_states: dict = {}   # loaded lazily

# ── Media group buffer ────────────────────────────────────────────────────────
# When a user sends multiple photos/videos at once Telegram delivers them as
# separate updates all sharing the same media_group_id. We buffer them for
# 3 seconds then copy all to Public Tips in one go.
_fd_media_buffer: dict = {}   # media_group_id → {"user_id":..,"msgs":[...],"expires":...}

def _fd_flush_media_groups(force_user_id=None, force_group_id=None):
    """
    Flush buffered media groups whose timer has expired.
    Can force-flush by user_id or specific group_id (used by background timer).
    Saves media refs into report_data and advances user to anon step.
    Media is NOT copied here — _fd_submit_report does the single copy on confirmation.
    Returns list of user_ids whose groups were flushed.
    """
    now = utcnow()
    flushed = []
    expired = [gid for gid, g in list(_fd_media_buffer.items())
               if (now - g["created_at"]).total_seconds() >= 3.0
               or (force_user_id and g["user_id"] == force_user_id)
               or (force_group_id and gid == force_group_id)]
    for gid in expired:
        group = _fd_media_buffer.pop(gid, None)
        if not group:
            continue
        uid  = group["user_id"]
        msgs = group.get("msgs", [])
        # Save all media refs into report_data — _fd_submit_report copies them
        # to Public Tips when the user confirms. Do NOT copy here to avoid duplicates.
        fd = _fd_state(uid)
        cur_rd = fd.get("report_data", {})
        cur_rd.setdefault("media", []).extend(group.get("media_items", []))
        _fd_save(uid, {"report_data": cur_rd, "report_step": "anon"})
        log.info(f"[FD] media group {gid}: buffered {len(msgs)} items for user {uid}")
        flushed.append(uid)
    return flushed

def _fd_state(user_id: int) -> dict:
    """Return current front desk state for a user. Creates default if missing."""
    uid = str(user_id)
    if uid not in _fd_states:
        # Try to restore from KV
        stored = kv_get(f"fd_state_{uid}", None)
        _fd_states[uid] = stored if isinstance(stored, dict) else {
            "mode": "main",
            "report_step": None,
            "report_data": {},
            "language_hint": "en",
            ...
            "last_seen": None,
            "last_options_shown_at": None,
        }
    return _fd_states[uid]

def _fd_save(user_id: int, updates: dict):
    """Apply updates to a user's front desk state and persist to KV."""
    uid = str(user_id)
    state = _fd_state(user_id)
    state.update(updates)
    state["last_seen"] = utcnow().isoformat()
    _fd_states[uid] = state
    try:
        kv_set(f"fd_state_{uid}", state)
    except Exception as _e:
        log.debug(f"[FD] state save failed for {uid}: {_e}")

def _fd_detect_lang(text: str) -> str:
    """Return 'dv', 'latin_dv', or 'en' for routing."""
    if is_dhivehi(text):
        return "dv"
    if looks_latin_thaana(text):
        return "latin_dv"
    return "en"

def _fd_is_inactive(state: dict) -> bool:
    """True if user has been idle for more than PUBLIC_INACTIVITY_MINUTES."""
    last = state.get("last_seen")
    if not last:
        return True
    try:
        from datetime import datetime, timezone
        last_dt = datetime.fromisoformat(last).replace(tzinfo=None)
        return (utcnow() - last_dt).total_seconds() > PUBLIC_INACTIVITY_MINUTES * 60
    except Exception:
        return True

def _fd_intent(text: str) -> str:
    """Detect report/news/chat intent from natural language (EN + DV)."""
    t = text.lower().strip()
    report_triggers = {
        "report","tip","i saw","accident happened","fire","flood","police",
        "send tip","i have news","something happened","ހާދިސާ","ރިޕޯޓް",
        "އެކްސިޑެންޓް","އަލިފާން","ހިނގި","ފެނިއްޖެ","ވިއްދާ",
    }
    news_triggers = {
        "news","latest","what happened","breaking","update","more","weather",
        "politics","sport","ޚަބަރު","ފަހުގެ","ބްރޭކިން","ލެޓެސްޓް","ކިޔައިދީ",
    }
    for kw in report_triggers:
        if kw in t:
            return "report"
...
def _fd_get_latest_news(lang: str = "en", limit: int = 5) -> str:
    """Pull latest posted articles from DB for the news button."""
    if not _db_ok():
        return "📰 <b>Latest Samuga updates</b>\n\nCould not connect to news archive right now. Follow <b>@samugacommunity</b> for live updates! 📡"
    try:
        rows = db_execute(
            """SELECT title, article_slug, lang
               FROM articles
               WHERE status = 'posted'
               ...
               LIMIT %s""",
            (lang if lang == "dv" else "en", limit), fetch="all"
        )
        if not rows:
            # Fallback: get any language
            rows = db_execute(
                "SELECT title, article_slug, lang FROM articles WHERE status='posted' "
                "AND title IS NOT NULL ORDER BY posted_at DESC LIMIT %s",
                (limit,), fetch="all"
            )
        if not rows:
            return "📰 No recent news in archive yet. Follow <b>@samugacommunity</b>! 📡"
        lines = ["📰 <b>Latest Samuga updates</b>\n"]
        for i, (title, slug, art_lang) in enumerate(rows, 1):
            clean_title = (title or "").strip()[:120]
            lines.append(f"{i}. {clean_title}")
        lines.append("\n<i>Send \"more\" for more updates or tap a button below.</i>")
        return "\n".join(lines)
    except Exception as e:
        log.warning(f"[FD] latest news query failed: {e}")
        return "📰 Follow <b>@samugacommunity</b> for live Maldives news! 📡"

def _fd_copy_media_to_tips(msg: dict, user_id: int, username: str):
    """Forward any media in a report message to the Public Tips thread."""
    try:
        import requests as _req
        source_chat = msg.get("chat", {}).get("id") or user_id
        ...
        _req.post(
            ...
            json={
                "chat_id": CORE_TEAM_CHAT_ID,
                "message_thread_id": PUBLIC_TIPS_THREAD_ID,
        ...
        log.warning(f"[FD] media copy to tips failed: {e}")

def _fd_submit_report(user_id: int, username: str, first_name: str, state: dict):
    """Send completed report to Public Tips thread."""
    rd = state.get("report_data", {})
    anon = rd.get("anonymous", True)
    identity = "Anonymous" if anon else f"@{username}" if username else f"User {user_id}"
    contact = "Yes" if rd.get("contact_allowed") else "No"
    tip_text = (
        f"🚨 <b>NEW PUBLIC TIP</b>\n\n"
        f"<b>Status:</b> New\n"
        f"<b>Location:</b> {rd.get('location','—')}\n"
        f"<b>Time:</b> {rd.get('incident_time','—')}\n"
        f"<b>Anonymous:</b> {'Yes' if anon else 'No'}\n"
        f"<b>Contact allowed:</b> {contact}\n\n"
        f"<b>Report:</b>\n{rd.get('description','—')}\n\n"
        f"<b>From:</b> {identity} (ID: <code>{user_id}</code>)\n\n"
        f"<i>🟡 Suggested action: Verify before publishing</i>"
    )
    send_text(CORE_TEAM_CHAT_ID, tip_text, thread_id=PUBLIC_TIPS_THREAD_ID)
    # Copy media in parallel — sequential HTTP calls were causing slowness
    ...
    if media_list:
        ...
        def _copy_one(m):
            try:
                ...
                log.warning(f"[FD] media copy failed: {_me}")
        threads = [threading.Thread(target=_copy_one, args=(m,), daemon=True) for m in media_list]
        for t in threads: t.start()
        for t in threads: t.join(timeout=12)  # wait max 12s for all
    log.info(f"[FD] tip submitted by user {user_id} ({identity}) — {len(media_list)} media item(s)")
...
# ── Samuga Cortex control layer (observe-first, safe release gate) ────────────
CORTEX_STATE_KEY = "samuga_cortex_state_v1"
CORTEX_PENDING_RELEASE_KEY = "samuga_cortex_pending_release_v1"
CORTEX_MODE_OBSERVE = "observe"   # posts analysis to Core Team only
CORTEX_MODE_ACTIVE  = "active"    # future: Cortex decisions may drive automation
CORTEX_MODE_OFF     = "off"       # no Cortex analysis messages

def _cx_safe(s, limit=None):
    """Escape HTML special chars for safe use inside Telegram HTML messages."""
    s = str(s or "")
...
def _cortex_state():
    """Return persisted Cortex state. Defaults to observe mode."""
    default = {
        "mode": CORTEX_MODE_OBSERVE,
        "released_by": None,
        "released_at": None,
        "updated_at": utcnow().isoformat(),
        "note": "Cortex observe mode: analysis only, no autonomous publishing changes.",
    }
    try:
        state = kv_get(CORTEX_STATE_KEY, default) or default
        if not isinstance(state, dict):
            return default
        state.setdefault("mode", CORTEX_MODE_OBSERVE)
        return state
    except Exception:
        return default

def _cortex_mode():
    return (_cortex_state().get("mode") or CORTEX_MODE_OBSERVE).lower()

def _set_cortex_mode(mode, by="Team", note=""):
    mode = (mode or CORTEX_MODE_OBSERVE).lower().strip()
    if mode not in (CORTEX_MODE_OBSERVE, CORTEX_MODE_ACTIVE, CORTEX_MODE_OFF):
        mode = CORTEX_MODE_OBSERVE
    state = {
        "mode": mode,
        "released_by": by if mode == CORTEX_MODE_ACTIVE else None,
        "released_at": utcnow().isoformat() if mode == CORTEX_MODE_ACTIVE else None,
        "updated_by": by,
        "updated_at": utcnow().isoformat(),
        "note": note or ("Cortex active mode." if mode == CORTEX_MODE_ACTIVE else "Cortex observe mode."),
    }
    try:
        kv_set(CORTEX_STATE_KEY, state)
    except Exception as e:
        log.error(f"Cortex state save failed: {e}")
    return state

def _cortex_status_text():
    state = _cortex_state()
    mode = state.get("mode", CORTEX_MODE_OBSERVE)
    availability = "✅ available" if CORTEX_AVAILABLE else "❌ not loaded (samuga_cortex.py missing)"
    released = ""
    if mode == CORTEX_MODE_ACTIVE:
        released = (f"\nReleased by: <b>{_cx_safe(state.get('released_by') or 'Team')}</b>"
                    f"\nReleased at: <code>{_cx_safe(state.get('released_at') or '')}</code>")
    return (
        f"🧠 <b>Samuga Cortex Status</b>\n\n"
        f"Engine: {availability}\n"
        f"Mode: <b>{_cx_safe(mode.upper())}</b>{released}\n\n"
        f"<b>Modes</b>\n"
        f"• <b>OBSERVE</b>: posts analysis to Core Team only; live bot unchanged.\n"
        f"• <b>ACTIVE</b>: release gate enabled for future autonomous Cortex decisions.\n"
        f"• <b>OFF</b>: no Cortex analysis messages.\n\n"
        f"Commands:\n"
        f"<code>/cortex release</code> — ask for confirmation before release\n"
        f"<code>/cortex observe</code> — return to safe observe mode\n"
        f"<code>/cortex off</code> — stop Cortex messages"
    )

def _cortex_begin_release(chat_id, thread_id, first_name):
    pending = {
    ...
    try:
        kv_set(CORTEX_PENDING_RELEASE_KEY, pending)
    except Exception as e:
        log.error(f"Cortex pending release save failed: {e}")
    kb = _make_inline_kb([
        [("✅ Confirm Cortex Release", "cortex_release_confirm")],
        [("❌ Cancel", "cortex_release_cancel")],
    ])
    msg = (
        f"🧠 <b>Release Samuga Cortex?</b>\n\n"
        f"Requested by: <b>{_cx_safe(first_name)}</b>\n\n"
        f"This will switch Cortex from <b>OBSERVE</b> to <b>ACTIVE</b>.\n"
        f"For safety, this does <b>not</b> release immediately. Confirm within 10 minutes.\n\n"
        f"Public posting logic remains protected by existing bot controls.\n\n"
        f"Or type: <code>/cortex confirm release</code>"
    )
    return send_text(chat_id, msg, thread_id=thread_id, reply_markup=kb)

def _cortex_confirm_release(chat_id, thread_id, first_name):
    try:
        pending = kv_get(CORTEX_PENDING_RELEASE_KEY, None)
    except Exception:
        pending = None
    if not pending:
        return send_text(chat_id, "⚠️ No pending Cortex release. Use <code>/cortex release</code> first.", thread_id=thread_id)
    try:
        exp = datetime.fromisoformat(str(pending.get("expires_at")))
        if utcnow() > exp:
            kv_set(CORTEX_PENDING_RELEASE_KEY, None)
            return send_text(chat_id, "⏰ Cortex release confirmation expired. Use <code>/cortex release</code> again.", thread_id=thread_id)
    except Exception:
        pass
    kv_set(CORTEX_PENDING_RELEASE_KEY, None)
    _set_cortex_mode(CORTEX_MODE_ACTIVE, by=first_name, note="Confirmed release from Core Team command.")
    return send_text(chat_id,
        f"🟢 <b>Samuga Cortex released.</b>\n\n"
        f"Confirmed by: <b>{_cx_safe(first_name)}</b>\n"
        f"Mode: <b>ACTIVE</b>\n\n"
        f"Cortex analysis will continue posting to Core Team. Existing public posting safeguards remain in place.",
        thread_id=thread_id)

def _cortex_cancel_release(chat_id, thread_id, first_name):
    try:
        kv_set(CORTEX_PENDING_RELEASE_KEY, None)
    except Exception:
        pass
    return send_text(chat_id, f"❌ Cortex release cancelled by <b>{_cx_safe(first_name)}</b>.", thread_id=thread_id)
...
# ── Dhivehi newsroom prompt ───────────────────────────────────────────────────
# Enforces formal journalistic Dhivehi (ލިޔުންތެރިކަމުގެ ރަސްމީ ބަސްމަގު).
# Modelled on Mihaaru / Sun editorial voice.
# Do NOT change lightly — this is the core voice of Samuga's Dhivehi output.
_DHIVEHI_WRITER_SYSTEM = """ތިބާ (You) ތި ވާ ހިމެނިގެންވަނީ ސަމުގާ މީޑިއާގެ ސީނިއަ ދިވެހި ނޫސްވެރިއެއްގެ ގޮތުގައެވެ.

You are the Senior Dhivehi News Editor for Samuga Media.
Your only task: write a clean, professional Dhivehi news caption in proper Thaana script.

CRITICAL EDITORIAL RULES:
1. TONE: Use formal authoritative journalistic Dhivehi only (ރަސްމީ ބަސްމަގު).
   Never use casual conversation style.
2. NO LITERAL TRANSLATION: Understand the event, then write it naturally as a
   Maldivian journalist would — not word-for-word from English.
3. VERB VARIETY: Use accurate verbs for the action:
   - Revealed/Announced → ހާމަކޮށްފިއެވެ
   - Informed/Stated    → މައުލޫމާތު ދީފިއެވެ / ވިދާޅުވިއެވެ
   - Noted/Highlighted  → ފާހަގަކޮށްފިއެވެ
   - Confirmed          → ކަށަވަރުކޮށްދީފިއެވެ
   - Arrested/Detained  → ހައްޔަރުކޮށްފިއެވެ
   - Occurred/Happened  → ހިނގައިގެންފިއެވެ / ހިނގި ހާދިސާ
   - Warned             → އިންޒާރު ދީފިއެވެ
4. STRUCTURE: Lead with the most important fact. Keep sentences short and clear.
   2–3 sentences maximum. No long winding paragraphs.
5. LANGUAGE PURITY:
   - Output ONLY Dhivehi Thaana script.
   - No Latin characters, no romanised Dhivehi, no English words.
   - Exception: official acronyms only (MIRA, MMA, MNDF, PSM, MDP, PPM, PNC).
6. NO ADDED FACTS: Do not invent details, add background, or make assumptions.
   Write only from the provided facts.
7. NO MARKDOWN: No asterisks, underscores, backticks, hyphens, or headers.
   Plain Thaana text only, ready to publish directly.

OUTPUT: Return ONLY the final Dhivehi caption. No preamble, no explanation, no quotes."""

def _get_dv_fewshot_examples(limit=3):
    """
    Pull real human-corrected Dhivehi captions from the learning table.
    Only returns examples where:
      - lang = 'dv'
      - action = 'edited'
      - original_caption != final_caption (real edit, not rubber-stamp)
      - final_caption contains proper Thaana script
    Returns list of (bad_text, good_text) tuples, or empty list if < 3 real edits.
    """
    if not _db_ok():
        return []
    try:
        rows = db_execute(
            """SELECT original_caption, final_caption
               FROM learning
               WHERE lang = 'dv'
                 AND action IN ('edited', 'approved')
                 AND original_caption IS NOT NULL
                 AND final_caption IS NOT NULL
                 AND original_caption != final_caption
                 AND LENGTH(final_caption) > 20
               ORDER BY id DESC
               LIMIT %s""",
            (limit * 3,), fetch="all"
        )
        if not rows:
            return []
        examples = []
        for orig, fixed in rows:
            if not orig or not fixed:
                continue
            if not is_dhivehi(fixed):
                continue  # final must be real Thaana
            if orig.strip() == fixed.strip():
                continue  # no real change
            examples.append((orig.strip()[:300], fixed.strip()[:300]))
            if len(examples) >= limit:
                break
        return examples
    except Exception as e:
        log.debug(f"[DV FEW-SHOT] DB query failed: {e}")
        return []

def clean_thaana_output(raw_text):
    """
    Post-process LLM Dhivehi output to remove structural artifacts.

    Cleans:
      - Markdown formatting (*, _, `, ~) that breaks RTL rendering
      - Common AI intro phrases ("Here is the caption:", "Summary:" etc.)
      - Wrapping quotes the model sometimes adds
      - Stray English/Latin sentences at start or end
      - Zero-width spaces and other Unicode junk
      - Multiple blank lines
    Returns cleaned string, or empty string if nothing valid remains.
    """
    if not raw_text:
        return ""
    ...
    # Strip common AI intro lines (English)
    s = re.sub(
        r"(?i)^(here is|here['']s|below is|the following is|dhivehi (caption|summary|translation|version)"
        r"|caption:|summary:|translation:|news:|output:)[:\s]*",
        "", s
    )
    ...
    # Remove stray English-only lines (lines with no Thaana chars)
    lines = s.splitlines()
    thaana_lines = [ln for ln in lines if is_dhivehi(ln) or not ln.strip() or
                    all(c.isspace() or "ހ" <= c <= "޿" for c in ln if c.strip())]
    # If filtering removed everything, keep original (failsafe)
    if any(is_dhivehi(ln) for ln in thaana_lines):
        s = "\n".join(thaana_lines)
...
def validate_dhivehi_public_text(text, title="", source=""):
    """
    Final gate before a Dhivehi caption enters Content Lab or goes public.

    Checks:
      1. Must contain actual Thaana characters
      2. Must not contain Latin Thaana (romanised Dhivehi)
      3. Must not contain source domains or URLs
      4. Must meet minimum length
      5. Must not be a placeholder/error message
    ...
    s = text.strip()
    if not is_dhivehi(s):
        return False, "no Thaana script found"
    if looks_latin_thaana(s):
        return False, "Latin Thaana detected in output"
    if re.search(r"https?://|\.mv/|\.com/|t\.me/|@[A-Za-z]", s, re.I):
        return False, "source link or domain in caption"
    if len(s) < 20:
        return False, f"too short ({len(s)} chars)"
    if not public_text_is_safe(s):
        return False, "placeholder or unsafe text"
    return True, ""

def make_dhivehi_caption(english_text, title, category="LOCAL"):
    """
    Dhivehi Writer v1 — convert news text to formal Dhivehi Thaana caption.

    Pipeline:
      _strip_telegram_metadata()
      strip_source_links()
      build prompt with:
        - strong editorial system instructions
        - few-shot examples from real editor corrections (if ≥3 exist)
        - source text
      call Gemini (primary) → clean_thaana_output() → validate_dhivehi_public_text()
      if Gemini fails → Claude Haiku fallback
      if both fail → return None (caller decides what to do)
    """
    if not GEMINI_API_KEY:
        return None

    # ── Pre-clean source text ─────────────────────────────────────────────────
    clean_text  = strip_source_links(_strip_telegram_metadata(english_text or ""))
    clean_title = strip_source_links(_strip_telegram_metadata(title or ""))
    if not clean_text and not clean_title:
        return None

    # ── Few-shot examples (only if real edits exist) ──────────────────────────
    fewshot_examples = _get_dv_fewshot_examples(limit=3)
    fewshot_block = ""
    if fewshot_examples:
        parts = ["SAMUGA EDITORIAL EXAMPLES (what good output looks like):"]
        for i, (bad, good) in enumerate(fewshot_examples, 1):
            parts.append(f"Example {i}:")
            parts.append(f"  Avoid this style: {bad}")
            parts.append(f"  Write like this:  {good}")
        fewshot_block = "\n".join(parts) + "\n\n"
        log.debug(f"[DV WRITER] Injecting {len(fewshot_examples)} few-shot example(s)")

    # ── Build prompt ──────────────────────────────────────────────────────────
    prompt = f"""{_DHIVEHI_WRITER_SYSTEM}

{fewshot_block}NEWS TO WRITE:

Title: {clean_title}
Summary: {clean_text}
Category: {category}

Write the Dhivehi caption now:"""

    # ── Primary: Gemini ───────────────────────────────────────────────────────
    raw = _gemini_post(prompt, timeout=20)
    if raw:
        cleaned = clean_thaana_output(raw)
        ok, reason = validate_dhivehi_public_text(cleaned, title=clean_title)
        if ok:
            log.info(f"✅ [DV WRITER] Gemini caption OK ({len(cleaned)} chars)")
            return cleaned
        log.warning(f"[DV WRITER] Gemini output rejected — {reason}: {repr(cleaned[:80])}")

    # ── Fallback: Claude Haiku ────────────────────────────────────────────────
    # Gemini failed or returned invalid Thaana. Try Claude Haiku as backup.
    # This is NOT a full replacement — Gemini is still primary.
    if ai:
        try:
            fallback_prompt = f"""You are the Senior Dhivehi News Editor for Samuga Media.
Write a 2–3 sentence news caption in proper Dhivehi Thaana script only.
Use formal journalistic tone (ރަސްމީ ބަސްމަގު). No English, no Latin Dhivehi, no markdown.
Rely strictly on the facts below. Do not add assumptions.

Title: {clean_title}
Summary: {clean_text}

Output ONLY the Dhivehi Thaana caption with no introduction:"""
            msg = ai.messages.create(
                model="claude-haiku-4-5-20251001",
                max_tokens=250,
                messages=[{"role": "user", "content": fallback_prompt}]
            )
            raw_fallback = msg.content[0].text.strip() if msg.content else ""
            cleaned_fb = clean_thaana_output(raw_fallback)
            ok_fb, reason_fb = validate_dhivehi_public_text(cleaned_fb, title=clean_title)
            if ok_fb:
                log.info(f"✅ [DV WRITER] Claude Haiku fallback caption OK ({len(cleaned_fb)} chars)")
                return cleaned_fb
            log.warning(f"[DV WRITER] Claude Haiku fallback rejected — {reason_fb}")
        except Exception as e:
            log.warning(f"[DV WRITER] Claude Haiku fallback failed: {e}")

    log.error(f"[DV WRITER] Both Gemini and Claude Haiku failed for: {clean_title[:60]}")
    return None
...
def _send_approval_card(key, item, force=False):
    ...
    if item["lang"] == "en":
        if item.get("is_breaking") and item.get("_held_for_confidence"):
            ...
            # Show the real auto-post timer based on score + confidence
            _card_score = article.get("_priority") or article.get("_score") or 0
            _card_conf  = article.get("_confidence") or 0
            if float(_card_score) >= ENGLISH_AUTOPOST_HIGH_SCORE and float(_card_conf) >= ENGLISH_AUTOPOST_HIGH_CONF:
                footer += "<i>⚡ Auto-posts in 5 min (high score + confidence)</i>"
            elif float(_card_conf) >= ENGLISH_AUTOPOST_MED_CONF:
                footer += "<i>⏰ Auto-posts in 15 min (medium confidence)</i>"
            else:
...
def post_article(article, seen, social_only=False, allow_social=True):
    ...
    # is_dv: proper Thaana articles + Latin Thaana articles that can be converted
    # _latin_dv flag is set by fetchers when text is romanised Dhivehi (MvCrisis etc.)
    # These go through the EN pipeline normally AND generate a DV card via Gemini
    _raw_lang   = article.get("lang", "en")
    _latin_flag = bool(article.get("_latin_dv"))
    is_dv = (_raw_lang == "dv") or (
        _latin_flag and looks_latin_thaana(
    ...
    # Use cached score if run_job already computed it this scan.
    priority = article.get("_score") if "_score" in article else score_article(article)
    article["_score"] = priority  # ensure it's set for downstream use
    ...
    if breaking and not is_dv:
        ...
        if tg_ok and GEMINI_API_KEY:
            def _auto_dv_breaking(_title=article["title"], _rewritten=rewritten,
                ...
                try:
                    dv_text = make_dhivehi_caption(_rewritten, _title, category=_cat if "_cat" in dir() else cat)
                    if not dv_text:
                        return
                    key = store_pending_approval(
    ...
    if is_dv:
        try:
            ...
            dv_text = make_dhivehi_caption(rewritten, article["title"], category=cat)
            if not dv_text:
...
def run_job(social_only=False, breaking_only=False):
    ...
    # ── Score cache: compute once per article per scan ────────────────────────
    # score_article() has no side effects but is not cheap. Caching on the dict
    # means run_job, progressive_funnel, and post_article all read the same
    # number without recomputing it 3-4x per article per scan.
    for a in fresh:
        if "_score" not in a:
            a["_score"] = score_article(a)

    fresh.sort(key=lambda a: a["_score"], reverse=True)
    ...
    # ── 1. BREAKING — fires immediately, but safety gates still apply ────────
    if breaking_articles:
        a = breaking_articles[0]
        # Gate 1: ad/junk filter — breaking path is NOT exempt
        _safe_ok, _safe_reason = contentlab_candidate_is_safe(
            title=a.get("title", ""),
        ...
        if not _safe_ok:
            log.warning(f"🔴 BREAKING blocked — {_safe_reason}: {a.get('title','')[:60]}")
        else:
            # Gate 2: minimum score (breaking still needs to be newsworthy)
            _bscore = a.get("_score", 0) or 0
            if _bscore > 0 and _bscore < 40:
                log.warning(f"🔴 BREAKING score too low ({_bscore}) — skipping: {a.get('title','')[:60]}")
            else:
                log.info(f"🔴 BREAKING: {a['title'][:60]}")
                post_article(a, seen, social_only=False, allow_social=True)

    # ── WEBSITE-FIRST (night only) ───────────────────────────────────────────
    # During DAY: EN winners are published to the website right before they
    # enter Content Lab (see the en_winners loop below).
    # During NIGHT: breaking_only=True skips the funnel entirely, so winners
    # never run. This block catches worthy EN articles overnight and puts them
    # on the website without going to Content Lab (which is closed at night).
    # Result: website is always 24/7 fresh, Content Lab flow unchanged.
    #
    # Quality gates applied (same as daytime):
    #   1. Score >= FUNNEL_MIN_SCORE (80) — only newsworthy articles
    #   2. contentlab_candidate_is_safe() — blocks junk, CSS, internal noise
    #   3. is_duplicate_story() — same event already on website → skip
    #   4. public_text_is_safe() — no placeholders after rewrite
    #   5. Not already in seen set — not processed in a prior scan
    if not breaking_only and not is_day_mode() and _db_ok():
        # Only run overnight — daytime publishing is handled by the en_winners loop
        _ws_regular = [a for a in fresh
                       if not is_breaking(a["title"], a.get("summary",""), a["cat"])
                       and a.get("lang","en") == "en"
                       and a["id"] not in seen]
        _website_night_count = 0
        for _wa in _ws_regular:
            try:
                # Gate 1: score threshold
                _wscore = _wa.get("_score", 0) or 0
                if _wscore < FUNNEL_MIN_SCORE:
                    continue

                # Gate 2: content safety (junk/CSS/internal text)
                _safe_ok, _safe_reason = contentlab_candidate_is_safe(
                    title=_wa.get("title",""),
                    summary=_wa.get("summary",""),
                    source=_wa.get("source",""),
                    lang=_wa.get("lang","en"),
                )
                if not _safe_ok:
                    log.debug(f"[WEBSITE] blocked — {_safe_reason}: {_wa.get('title','')[:50]}")
                    continue

                # Gate 3: dedup — same event already on website?
                _dedup_key = _wa.get("_dedup_title") or story_signal_key(
                    _wa.get("title",""), _wa.get("summary",""), _wa.get("lang","en")
                )
                if _dedup_key and is_duplicate_story(_dedup_key):
                    log.debug(f"[WEBSITE] duplicate skipped: {_wa.get('title','')[:55]}")
                    seen.add(_wa["id"]); save_seen(seen)
                    continue

                # Gate 4: rewrite + public text safety
                _wrewritten, _wkeyword = rewrite_news(
                    _strip_telegram_metadata(_wa["title"]),
                    _strip_telegram_metadata(_wa.get("summary", "")),
                    _wa["cat"]
                )
                if not public_text_is_safe(_wrewritten):
                    continue

                # All gates passed — publish to website
                db_publish_article_for_website(
                    article_id=_wa["id"],
                    title=_wa["title"],
                    summary=samuga_public_summary(_wa.get("title",""), _wa.get("summary",""), _wrewritten),
                    category=canonical_category(_wa["cat"], _wa["title"], _wa.get("summary","")),
                    source=SAMUGA_PUBLIC_SOURCE,
                    link=SAMUGA_PUBLIC_LINK,
                    lang="en",
                    score=int(_wscore),
                    reliability=source_reliability(_wa.get("source","")),
                    is_breaking=False,
                )
                if _dedup_key:
                    remember_story_title(_dedup_key)
                seen.add(_wa["id"]); save_seen(seen)
                _website_night_count += 1
                log.info(f"🌐 [WEBSITE 24/7] {_wa.get('title','')[:60]} (score={int(_wscore)})")
            except Exception as _wne:
                log.debug(f"[WEBSITE] publish failed: {_wa.get('title','')[:50]}: {_wne}")
        if _website_night_count:
            log.info(f"🌐 [WEBSITE 24/7] Published {_website_night_count} articles overnight")
    ...
    def progressive_funnel(articles, label=""):
        ...
        # Use cached _score if run_job already computed it; avoids double-scoring.
        scored = []
        for a in articles:
            s = a.get("_score") if "_score" in a else score_article(a)
        ...
        while len(pool) > 3:
            ...
            # We clear the cache here intentionally so the new cluster size
            # is reflected — this is the one place re-scoring is worthwhile.
            for a in pool:
                a["_score"] = score_article(a)
                a["_funnel_score"] = a["_score"]
    ...
    for a in en_winners:
        ...
        if not is_duplicate_story(a["title"]):
            log.info(f"🟡 EN winner (score {a.get('_funnel_score',0)}) → Content Lab + Website: {a['title'][:55]}")
            # ── Website publish: every EN winner goes to website immediately ──
            # This runs BEFORE post_article() so the article is live on the
            # website the moment it enters Content Lab — no waiting for approval.
            # post_article() still handles Telegram/social/card creation as before.
            if _db_ok():
                try:
                    _wcat = canonical_category(a["cat"], a["title"], a.get("summary",""))
                    _wrel = source_reliability(a.get("source",""))
                    _wscore = int(a.get("_funnel_score") or a.get("_score") or 0)
                    db_publish_article_for_website(
                        article_id=a["id"],
                        title=a["title"],
                        summary=samuga_public_summary(a.get("title",""), a.get("summary",""), a.get("summary","")),
                        category=_wcat,
                        source=SAMUGA_PUBLIC_SOURCE,
                        link=SAMUGA_PUBLIC_LINK,
                        lang="en",
                    ...
                    log.info(f"🌐 EN winner published to website: {a['title'][:55]}")
                except Exception as _wex:
                    log.warning(f"[WEBSITE] EN winner publish failed: {_wex}")
            post_article(a, seen, social_only=False, allow_social=True)
            en_sent += 1
        else:
            note_duplicate_skip()
...
def fetch_meta_insights(days=28):
    ...
    """
    if not _db_ok() or not META_PAGE_TOKEN:
        return 0
    ...
    def publish_approved_item(item, key, approver="Team", corrected=None, destination="all"):
        ...
        try:
            ...
            if item["lang"] == "dv":
                ...
                # ── Learning loop: record this editor action ──────────────────
                item["_queue_key"] = key
                _bm_log("edited" if corrected else "post_all", item=item,
                        member=approver, extra=destination)
            else:
                ...
                # ── Learning loop: record this editor action ──────────────────
                item["_queue_key"] = key
                _bm_log(_bm_action_for(destination), item=item,
                        member=approver, extra=destination)
        ...
        try:
            ...
            for update in resp.json().get("result",[]):
                ...
                if cbq:
                    ...
                    if cbq_data in ("cortex_release_confirm", "cortex_release_cancel"):
                        if cbq_data == "cortex_release_confirm":
                            answer_callback_query(cbq_id, "🧠 Releasing Cortex...")
                            edit_message_reply_markup(cbq_chat_id, cbq_msg_id, {"inline_keyboard": []})
                            _cortex_confirm_release(cbq_chat_id, CONTENT_LAB_THREAD_ID, cbq_name)
                        else:
                            answer_callback_query(cbq_id, "❌ Cancelled")
                            edit_message_reply_markup(cbq_chat_id, cbq_msg_id, {"inline_keyboard": []})
                            _cortex_cancel_release(cbq_chat_id, CONTENT_LAB_THREAD_ID, cbq_name)
                        offset = update["update_id"] + 1
                        _poll_offset[0] = offset
                        continue

                    # ── Front Desk button callbacks ───────────────────────
                    if cbq_data.startswith("fd:"):
                        fd_action = cbq_data[3:]
                        fd_user_id = cbq_user.get("id", 0)
                        fd_chat_id = cbq_chat_id
                        fd_first   = cbq_user.get("first_name", "")
                        fd_uname   = cbq_user.get("username", "")
                        fd_st      = _fd_state(fd_user_id)
                        answer_callback_query(cbq_id)

                        if fd_action == "news":
                            lang_h = fd_st.get("language_hint", "en")
                            news_text = _fd_get_latest_news(lang=lang_h)
                            send_text(fd_chat_id, news_text)
                            _fd_save(fd_user_id, {"mode": "main"})

                        elif fd_action == "report":
                            _fd_save(fd_user_id, {"mode": "report",
                                                   "report_step": "what_happened",
                                                   "report_data": {}})
                            send_text(fd_chat_id,
                                "🚨 <b>What happened?</b>\n\n"
                                "Please write a short message about what you saw.\n"
                                "<i>Example: \"Accident near Hulhumalé Phase 2\"</i>")

                        elif fd_action == "chat":
                            _fd_save(fd_user_id, {"mode": "chat"})
                            send_text(fd_chat_id,
                                "💬 Ask me anything about news, Maldives updates, or Samuga Media.")

                        elif fd_action == "anon_yes":
                            rd = fd_st.get("report_data", {})
                            rd["anonymous"] = True
                            rd["contact_allowed"] = False
                            _fd_submit_report(fd_user_id, fd_uname, fd_first,
                                              {**fd_st, "report_data": rd})
                            _fd_save(fd_user_id, {"mode": "main", "report_step": None,
                                                   "report_data": {}})
                            send_text(fd_chat_id,
                                "✅ <b>Your report has been sent to Samuga Media.</b>\n\n"
                                "Our team will review and confirm before publishing.\n\n"
                                "What would you like to do next?",
                                reply_markup=_fd_kb("after_report"))

                        elif fd_action == "anon_no":
                            rd = fd_st.get("report_data", {})
                            rd["anonymous"] = False
                            rd["contact_allowed"] = True
                            _fd_submit_report(fd_user_id, fd_uname, fd_first,
                                              {**fd_st, "report_data": rd})
                            _fd_save(fd_user_id, {"mode": "main", "report_step": None,
                                                   "report_data": {}})
                            send_text(fd_chat_id,
                                "✅ <b>Your report has been sent to Samuga Media.</b>\n\n"
                                "Our team may reach out for more details.\n\n"
                                "What would you like to do next?",
                                reply_markup=_fd_kb("after_report"))
                    ...
                    if (cbq_data.startswith("post_tg:") or cbq_data.startswith("post_soc:")
                        ...
                        if action in DEST_MAP and cb_key in approval_queue:
                            ...
                            # ── Learning loop ─────────────────────────────────
                            item["_queue_key"] = cb_key
                            _bm_log("reject", item=item, member=cbq_name, extra="button_reject")
                ...
                # ── Flush expired media groups before processing this update ──
                _flushed_uids = _fd_flush_media_groups()
                for _fuid in _flushed_uids:
                    # Send anon question now that all media in the group is collected
                    _ffd = _fd_state(_fuid)
                    if _ffd.get("report_step") == "anon":
                        send_text(int(_fuid),
                            "🕵️ <b>Do you want to stay anonymous?</b>\n\n"
                            "Your identity will <b>not</b> be posted publicly.",
                            reply_markup=_fd_kb("anon"))
                ...
                media_group_id = msg.get("media_group_id")  # set when user sends album
                ...
                # Skip only if truly empty (no text AND no media of any kind)
                video_note = msg.get("video_note")
                document   = msg.get("document")
                voice      = msg.get("voice")
                if not text and not photo and not video and not video_note and not document and not voice:
                    continue
                if not text: text=""
                # Skip videos for card creation — only photos supported for cards
                ...
                if chat_type=="private":
                    # ── Samuga Media Front Desk ─────────────────────────────────
                    fd = _fd_state(user_id)
                    lang_hint = _fd_detect_lang(text)
                    _fd_save(user_id, {"language_hint": lang_hint})

                    # ── /start or first-time user ──────────────────────────────
                    if text.startswith("/start") or not fd.get("has_seen_welcome"):
                        _fd_save(user_id, {"has_seen_welcome": True, "mode": "main",
                                           "report_step": None, "report_data": {}})
                        send_text(chat_id, _FD_WELCOME, reply_markup=_fd_kb("main"))
                        continue

                    # ── /search command (pass-through, rate limited) ───────────
                    if text.startswith("/search "):
                        allowed, count, limit = dm_check_and_increment(user_id)
                        if not allowed:
                            send_text(chat_id,
                                f"You've reached today's limit of {limit} messages 🙏\n\n"
                                f"Come back tomorrow! Follow <b>@samugacommunity</b> for live news. 📡",
                                reply_to=msg_id)
                        else:
                            query = text[8:].strip()
                    ...
                    # ── Rate limit gate (applies to all modes) ────────────────
                    mode = fd.get("mode", "main")
                    report_step = fd.get("report_step")

                    # ── REPORT FLOW — no rate limit, no AI calls ───────────────
                    # Report steps are simple text saves, not AI chat messages.
                    # Skip dm_check_and_increment so report flow is instant.
                    if mode == "report" and report_step:
                        rd = fd.get("report_data", {})

                        if report_step == "what_happened":
                            rd["description"] = text
                            _fd_save(user_id, {"report_data": rd, "report_step": "where"})
                            send_text(chat_id,
                                "📍 <b>Where did it happen?</b>\n\n"
                                "Please send the island, area, or exact location.\n"
                                "<i>Example: \"Hulhumalé Phase 2 near Hiyaa flats\"</i>",
                                reply_to=msg_id)

                        elif report_step == "where":
                            rd["location"] = text
                            _fd_save(user_id, {"report_data": rd, "report_step": "when"})
                            send_text(chat_id,
                                "🕒 <b>When did it happen?</b>\n\n"
                                "<i>Example: \"Just now\" / \"About 10 minutes ago\" / \"Today morning\"</i>",
                                reply_to=msg_id)

                        elif report_step == "when":
                            rd["incident_time"] = text
                            _fd_save(user_id, {"report_data": rd, "report_step": "media"})
                            send_text(chat_id,
                                "📸 <b>Do you have a photo or video?</b>\n\n"
                                "You can send <b>one or multiple</b> photos and videos — "
                                "just select them all at once from your gallery before sending.\n\n"
                                "Or type <b>skip</b> to continue without media.",
                                reply_to=msg_id)

                        elif report_step == "media":
                            # Accept photo/video/video_note(round)/document/voice
                            media_item = None
                            for mtype in ("photo", "video", "video_note", "document", "voice"):
                                if msg.get(mtype):
                                    media_item = {"type": mtype, "chat_id": chat_id, "message_id": msg_id}
                                    break

                            if media_item:
                                if media_group_id:
                                    # ── Album/multi-media: buffer all items, flush after 3s ──
                                    # Telegram sends each photo/video in an album as a separate
                                    # update with the same media_group_id. We collect them all
                                    # then copy them to Public Tips in one pass.
                                    if media_group_id not in _fd_media_buffer:
                                        _fd_media_buffer[media_group_id] = {
                                            "user_id":     user_id,
                                            "report_data": rd,
                                            "msgs":        [],
                                            "media_items": [],
                                    ...
                                    _fd_media_buffer[media_group_id]["msgs"].append(
                                        {"chat_id": chat_id, "message_id": msg_id}
                                    )
                                    _fd_media_buffer[media_group_id]["media_items"].append(media_item)
                                    # First item — acknowledge + spawn background timer
                                    if len(_fd_media_buffer[media_group_id]["msgs"]) == 1:
                                        send_text(chat_id, "📎 Collecting all your media...", reply_to=msg_id)
                                        # Background thread fires after 3s regardless of user activity
                                        # This ensures the anon question always appears promptly
                                        def _delayed_flush(_gid=media_group_id, _cid=chat_id, _uid=user_id):
                                            import time as _t; _t.sleep(3)
                                            flushed = _fd_flush_media_groups(force_group_id=_gid)
                                            if _uid in flushed:
                                                send_text(_cid,
                                                    "🕵️ <b>Do you want to stay anonymous?</b>\n\n"
                                                    "Your identity will <b>not</b> be posted publicly.",
                                    ...
                                    # Don't advance state here — timer thread handles it
                                else:
                                    # ── Single media item — copy immediately ──
                                    try:
                                        _fd_copy_media_to_tips(msg, user_id, username or "")
                                    except Exception as _mce:
                                        log.warning(f"[FD] media copy failed: {_mce}")
                                    rd.setdefault("media", []).append(media_item)
                                    _fd_save(user_id, {"report_data": rd, "report_step": "anon"})
                                    send_text(chat_id, "✅ Got it! Media saved.", reply_to=msg_id)
                                    send_text(chat_id,
                                        "🕵️ <b>Do you want to stay anonymous?</b>\n\n"
                                        "Your identity will <b>not</b> be posted publicly.",
                                        reply_markup=_fd_kb("anon"))
                            else:
                                # No media — skip or unrecognised text
                                _fd_save(user_id, {"report_data": rd, "report_step": "anon"})
                                send_text(chat_id,
                                    "🕵️ <b>Do you want to stay anonymous?</b>\n\n"
                                    "Your identity will <b>not</b> be posted publicly.",
                    ...
                    # ── Detect natural intent if in main/chat mode ─────────────
                    if mode in ("main", "chat"):
                        intent = _fd_intent(text)

                        if intent == "news":
                            news_text = _fd_get_latest_news(lang=lang_hint)
                            send_text(chat_id, news_text, reply_to=msg_id)
                            # Show small menu if they were idle (natural stopping point)
                            if _fd_is_inactive(fd):
                                send_text(chat_id, "What else can I help you with?",
                                          reply_markup=_fd_kb("small"))
                                _fd_save(user_id, {"last_options_shown_at": utcnow().isoformat()})
                            continue

                        if intent == "report":
                            _fd_save(user_id, {"mode": "report", "report_step": "what_happened",
                                               "report_data": {}})
                            send_text(chat_id,
                                "🚨 <b>What happened?</b>\n\n"
                                "Please write a short message about what you saw.\n"
                    ...
                    # ── AI chat (default) — rate limit applies here ────────────
                    allowed, count, limit = dm_check_and_increment(user_id)
                    if not allowed:
                        send_text(chat_id,
                            f"You've reached today's limit of {limit} messages 🙏\n\n"
                            f"Come back tomorrow! Follow <b>@samugacommunity</b> for live Maldives news. 📡",
                            reply_to=msg_id)
                        log.info(f"🚫 DM rate limit: {display_name} ({user_id})")
                        continue
                    _fd_save(user_id, {"mode": "chat"})
                    log.info(f"💬 FD chat [{lang_hint}] {display_name} [{count}/{limit}]: {text[:50]}")
                    try:
                        ai_lang = "dv" if lang_hint in ("dv", "latin_dv") else "en"
                        reply = public_samuga_ai_chat(
                            message=text, platform="telegram",
                            user_key=user_id, session_id=str(chat_id), lang=ai_lang
                        )
                    except Exception as e:
                        log.error(f"[FD] AI chat failed: {e}")
                        reply = "Small issue on my side 😅 Try again in a moment."
                    send_text(chat_id, reply, reply_to=msg_id)
                    # Show small re-engage menu after inactivity
                    if _fd_is_inactive(fd):
                        send_text(chat_id,
                            "📰 Latest news  •  🚨 Report  •  💬 Chat",
                            reply_markup=_fd_kb("small"))
                        _fd_save(user_id, {"last_options_shown_at": utcnow().isoformat()})
                    ...
                    if is_core_team:
                        ...
                        # ── EARLY Cortex command gate ─────────────────────────────────────
                        # Handled first so the long elif chain below can never swallow it.
                        # Uses text_cmd_low (already @SamugaNewsBot-stripped) so both
                        # /cortex status  and  /cortex@SamugaNewsBot status  work correctly.
                        _core_cmd = text_cmd_low.strip()
                        if _core_cmd.startswith("/cortex") or _core_cmd in ["/cx", "/brain", "/cortexstatus", "/cortex_status"]:
                            log.info(f"🧠 Cortex command from {first_name}: {_core_cmd}")
                            if _core_cmd in ["/cortex", "/cortex status", "/cx", "/brain", "/cortexstatus", "/cortex_status"]:
                                send_text(chat_id, _cortex_status_text(), reply_to=msg_id, thread_id=thread_id)
                            elif _core_cmd in ["/cortex release", "/cortex active"]:
                                _cortex_begin_release(chat_id, thread_id, first_name)
                            elif _core_cmd in ["/cortex confirm release", "/cortex release confirm", "/cortex confirm"]:
                                _cortex_confirm_release(chat_id, thread_id, first_name)
                            elif _core_cmd in ["/cortex cancel", "/cortex cancel release", "/cortex release cancel"]:
                                _cortex_cancel_release(chat_id, thread_id, first_name)
                            elif _core_cmd in ["/cortex observe", "/cortex safe", "/cortex watch"]:
                                _set_cortex_mode(CORTEX_MODE_OBSERVE, by=first_name, note="Returned to observe mode by command.")
                                send_text(chat_id, "🟡 Samuga Cortex is now in <b>OBSERVE</b> mode. Analysis only, no public posting control.", reply_to=msg_id, thread_id=thread_id)
                            elif _core_cmd in ["/cortex off", "/cortex stop"]:
                                _set_cortex_mode(CORTEX_MODE_OFF, by=first_name, note="Cortex messages turned off by command.")
                                send_text(chat_id, "⚫ Samuga Cortex messages are now <b>OFF</b>.", reply_to=msg_id, thread_id=thread_id)
                            else:
                                send_text(chat_id,
                                    "Use: <code>/cortex status</code> · <code>/cortex release</code> · <code>/cortex observe</code> · <code>/cortex off</code>",
                                    reply_to=msg_id, thread_id=thread_id)
                            continue  # skip rest of command chain for this update
                        ...
                        # /reject <key>
                            ...
                            if key in approval_queue:
                                ...
                                # ── Learning loop: record rejection ───────────
                                rej_item["_queue_key"] = key
                                _bm_log("reject", item=rej_item,
                                        member=first_name, extra="reject")
                                try:
...
# /hide_dv — hide all currently posted Dhivehi website articles
                                ...
                                try:
                                    ...
                                    for f in dv_feeds:
                                        try:
                                            _furl = f.get("url","")
                                            if not _furl:
                                                continue  # skip env var feeds not yet configured
                                            parsed = feedparser.parse(_furl)
                                            n = len(parsed.entries)
                                            _parts = _furl.split("/")
                                            domain = _parts[2] if len(_parts) > 2 else _furl[:40]
                                            status = f"✅ {n} items" if n > 0 else "❌ 0 items (blocked/down)"
                                            lines.append(f"  {status} — {domain}")
                                        except Exception as fe:
                                            _furl2 = f.get("url","")
                                            _p2 = _furl2.split("/")
                                            _dom = _p2[2] if len(_p2) > 2 else _furl2[:30]
                                            lines.append(f"  ❌ {_dom}: error")
                        ...
                        # /why <key> — explain how a queued card scored
                        elif text.strip().lower().startswith("/why") or text_cmd_low.startswith("/cx why"):
                            # /why <key>  OR  /cx why <key>
                            raw = text_cmd_low.strip()
                            if raw.startswith("/cx why"):
                                key = raw[7:].strip()
                            else:
                            ...
                            if not key:
                                send_text(chat_id,
                                    "Usage: <code>/why en12</code> or <code>/cx why en12</code>\n"
                                    "Shows scoring breakdown + Cortex journalist analysis for a queued card.\n"
                                    "Run <code>/pending</code> to see keys.",
                                    reply_to=msg_id, thread_id=thread_id)
                                ...
                                art = {
                                    ...
                                    "_cluster_sources": item.get("_cluster_sources", []),
                                    "_trend_theme": item.get("_trend_theme",""),
                                    "_priority":    item.get("_priority", 0),
                                    "_confidence":  item.get("_confidence", 0),
                                }
                                def _run_why(_chat=chat_id, _thread=thread_id, _key=key, _art=art, _item=item, _msg=msg_id):
                                    try:
                                        # ── Part 1: existing score breakdown ──
                                        score_block = format_score_breakdown(_art)
                                        send_text(_chat, score_block, reply_to=_msg, thread_id=_thread)

                                        # ── Part 2: Cortex journalist analysis ──
                                        if not CORTEX_AVAILABLE or not cortex_decide:
                                            send_text(_chat,
                                                "🧠 <b>Cortex not loaded</b> — make sure samuga_cortex.py is deployed.",
                                                thread_id=_thread)
                                            return
                                        decision = cortex_decide({
                                            "title":         _art["title"],
                                            "summary":       _art["summary"],
                                            "body":          _art["summary"],
                                            "source":        _art["source"],
                                            "cat":           _art["cat"],
                                            "category":      _art["cat"],
                                            "lang":          _art["lang"],
                                            "_cluster_size": _art["_cluster_size"],
                                            "_cluster_sources": _art.get("_cluster_sources",[]),
                                        })
                                        if not decision:
                                            send_text(_chat, "🧠 Cortex returned no analysis.", thread_id=_thread)
                                            return

                                        # Format the Cortex block
                                        conf_bar = "🟢" if decision.confidence_score >= 70 else ("🟡" if decision.confidence_score >= 45 else "🔴")
                                        risk_bar = "🔴" if decision.risk_score >= 60 else ("🟡" if decision.risk_score >= 30 else "🟢")

                                        lines = [
                                            f"🧠 <b>Cortex Analysis — {_key.upper()}</b>",
                                            f"",
                                            f"<b>Decision:</b> <code>{decision.decision}</code>",
                                            f"<b>Story type:</b> {decision.story_type}",
                                            f"<b>Score:</b> {decision.final_score}/100  "
                                            f"{conf_bar} Confidence: {decision.confidence_score}%  "
                                            f"{risk_bar} Risk: {decision.risk_score}%",
                                            f"<b>Breaking:</b> {'🚨 YES' if decision.breaking else 'No'}",
                                            f"",
                                            f"<b>📐 Angle for writing:</b>",
                                            f"<i>{decision.angle}</i>",
                                        ]

                                        if decision.reasons:
                                            lines += ["", "<b>Why Cortex picked this:</b>"]
                                            lines += [f"• {r}" for r in decision.reasons[:5]]

                                        if decision.penalties:
                                            lines += ["", "<b>⚠️ Caution / penalties:</b>"]
                                            lines += [f"• {p}" for p in decision.penalties[:4]]

                                        if decision.truth_matrix.missing_questions:
                                            lines += ["", "<b>❓ Missing before publishing:</b>"]
                                            lines += [f"• {q}" for q in decision.truth_matrix.missing_questions[:3]]

                                        if decision.truth_matrix.unverified_claims:
                                            lines += ["", "<b>🔍 Unverified / alleged:</b>"]
                                            lines += [f"• {c[:80]}" for c in decision.truth_matrix.unverified_claims[:2]]

                                        # Learned boost from brain_memory
                                        if BRAIN_MEMORY_AVAILABLE and _bm_boost:
                                            try:
                                                boost_total, boost_parts = _bm_boost(
                                                    source=_art["source"],
                                                    category=_art["cat"].lower(),
                                                    story_type=decision.story_type,
                                                    language=_art["lang"],
                                                    title=_art["title"],
                                                    summary=_art["summary"],
                                                )
                                                if boost_total != 0:
                                                    top_keys = sorted(boost_parts.items(), key=lambda x: abs(x[1]), reverse=True)[:3]
                                                    boost_str = ", ".join(f"{k}: {v:+.1f}" for k, v in top_keys if v != 0)
                                                    lines += ["", f"<b>🧠 Learned boost:</b> <code>{boost_total:+.1f}</code>"]
                                                    if boost_str:
                                                        lines.append(f"<i>{boost_str}</i>")
                                            except Exception as _be:
                                                log.debug(f"/why learned boost: {_be}")

                                        lines += ["", f"━━━━━━━━━━━━━━"]
                                        lines.append(f"<code>/approved {_key}</code>  ·  <code>/reject {_key}</code>")

                                        send_text(_chat, "\n".join(lines), thread_id=_thread)
                                        log.info(f"🧠 /why Cortex block sent for {_key}")

                                    except Exception as e:
                                        log.error(f"/why cortex: {e}")
                                        send_text(_chat, f"❌ Cortex analysis failed for {_key}: {str(e)[:120]}", thread_id=_thread)

                                threading.Thread(target=_run_why, daemon=True).start()
                        ...
                        # /cortex — safety-net fallback handler
                        # The early gate above handles this via 'continue' in nearly all
                        # cases. This elif catches any edge-case path that reaches here.
                        # Uses text_cmd_low (already @-stripped) — never raw text — so
                        # /cortex@SamugaNewsBot subcommand matching always works correctly.
                        elif text_cmd_low.startswith("/cortex"):
                            _cmd = text_cmd_low.strip()
                            if _cmd in ["/cortex", "/cortex status"]:
                                send_text(chat_id, _cortex_status_text(), reply_to=msg_id, thread_id=thread_id)
                            elif _cmd in ["/cortex release", "/cortex active"]:
                                _cortex_begin_release(chat_id, thread_id, first_name)
                            elif _cmd in ["/cortex confirm release", "/cortex release confirm", "/cortex confirm"]:
                                _cortex_confirm_release(chat_id, thread_id, first_name)
                            elif _cmd in ["/cortex cancel", "/cortex cancel release", "/cortex release cancel"]:
                                _cortex_cancel_release(chat_id, thread_id, first_name)
                            elif _cmd in ["/cortex observe", "/cortex safe", "/cortex watch"]:
                                _set_cortex_mode(CORTEX_MODE_OBSERVE, by=first_name, note="Returned to observe mode by command.")
                                send_text(chat_id, "🟡 Samuga Cortex is now in <b>OBSERVE</b> mode. Analysis only, no public posting control.", reply_to=msg_id, thread_id=thread_id)
                            elif _cmd in ["/cortex off", "/cortex stop"]:
                                _set_cortex_mode(CORTEX_MODE_OFF, by=first_name, note="Cortex messages turned off by command.")
                                send_text(chat_id, "⚫ Samuga Cortex messages are now <b>OFF</b>.", reply_to=msg_id, thread_id=thread_id)
                            else:
                                send_text(chat_id,
...
def ensure_article_engine_body(article_id, title, summary, category,
    ...
    Fallback chain (each tried in order until something non-empty comes back):
      1. generate_website_article_body() — Claude-written full article
      2. summary (the rewritten card text we already have)
      3. title alone (absolute last resort — at least the page isn't blank)

    This means the website never serves a blank body, even if Claude is down
    or generate_website_article_body raises an exception.
    """
    body = ""
    # ── Attempt 1: full AI-generated article body ─────────────────────────────
    try:
        body = (generate_website_article_body(
        ...
        log.warning(f"[WEBSITE] body generation failed ({type(e).__name__}): {e}")
        body = ""

    # ── Attempt 2: use the rewritten summary we already have ──────────────────
    if not body or len(body) < 60:
        fallback = (summary or "").strip()
        if fallback and len(fallback) >= 40:
            log.info(f"[WEBSITE] body gen short/empty — using summary fallback ({len(fallback)} chars)")
            body = fallback

    # ── Attempt 3: absolute last resort — title only ──────────────────────────
    if not body:
        log.warning(f"[WEBSITE] summary also empty — using title-only fallback for: {str(title)[:80]}")
        body = (title or "").strip()

    # ── Persist so we don't regenerate on every API request ───────────────────
    try:
