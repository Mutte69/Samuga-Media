...
function openView(name, options = {}) {
  ...
  if (name === "contentlab") { setContentLabLoading(); requestContentLabRefresh(true); contentLabRefreshTimer = setInterval(() => { if (document.visibilityState === "visible") requestContentLabRefresh(false); }, 8000); }
...
function contentLabInteractionActive() {
  ...
  return Boolean(
    active?.matches?.(".lab-edit-text") ||
    $("#contentLabList .lab-edit-box.open") ||
    $("#contentLabList .lab-working") ||
    $('#contentLabList .lab-edit-text[data-dirty="true"]')
  );
}
...
function renderContentLab(items) {
  ...
  container.innerHTML = items.map(item => {
    const serverCopy = item.lang === "dv" ? (item.dv_text || item.summary || "") : (item.caption || item.summary || "");
    const copy = contentLabDrafts.has(item.key) ? contentLabDrafts.get(item.key) : serverCopy;
  ...
  $$('[data-lab-edit]', container).forEach(button => button.addEventListener("click", () => {
    const editBox = button.closest(".lab-card-body").querySelector(".lab-edit-box");
    if (!editBox) return;
    editBox.classList.toggle("open");
    button.textContent = editBox.classList.contains("open") ? "✕ Close edit" : "✏️ Edit";
    if (!editBox.classList.contains("open") && contentLabPendingRefresh) {
      contentLabPendingRefresh = false;
      requestContentLabRefresh(false);
    }
  }));
  $$('.lab-edit-text', container).forEach(textarea => textarea.addEventListener("input", () => {
    const card = textarea.closest("[data-lab-key]");
    if (!card) return;
    contentLabDrafts.set(card.dataset.labKey, textarea.value);
...
async function runContentLabAction(card, action, corrected = "") {
  ...
  try {
    ...
    contentLabDrafts.delete(key); toast(data.message || `${String(key).toUpperCase()} action accepted`); await loadContentLab({force:true});
