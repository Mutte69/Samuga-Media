/**
 * Samuga Media — Cloudflare Pages Worker (no redirect loop)
 *
 * This version DOES NOT fetch /article.html from Cloudflare assets.
 * That avoids Cloudflare Pages clean-url redirects like article.html → /article,
 * which can create ERR_TOO_MANY_REDIRECTS.
 *
 * /article?id=... returns a complete server-rendered article page for both
 * humans and social crawlers, with real Open Graph cover image tags.
 */
...
async function getArticle(id) {
  const response = await fetch(`${RAILWAY_API}/api/article?id=${encodeURIComponent(id)}`, {
    headers: {
      ...
      "User-Agent": "SamugaMedia-CloudflarePagesWorker/2.0"
    },
...
<body>
  ...
  <header class="article-header" role="banner">
    <div class="wrap article-header-inner">
      <a class="article-header-brand" href="/" aria-label="Samuga Media">
        <img src="/assets/Samuga_Media_Logo_Transparent.png" alt="Samuga Media" style="height:28px;width:auto;max-width:140px;object-fit:contain;border-radius:0;filter:drop-shadow(0 0 8px rgba(40,184,255,.2))" onerror="this.style.display='none'">
      </a>
      <nav class="article-header-nav">
        <a class="header-btn" href="/">← Latest</a>
  ...
  <main class="article-main">
    ${data.cover_image ? `<div class="article-cover"><img src="${esc(image)}" alt="${esc(coverCaption)}" loading="eager"><div class="article-cover-fade"></div>${data.cover_caption ? `<div class="article-cover-cap">${esc(data.cover_caption)}</div>` : ""}</div>` : `<div class="article-no-cover"></div>`}
    <div class="article-container">
      <article aria-label="Article" dir="${isDv ? "rtl" : "ltr"}">
        <header class="article-hero">
          <div class="article-eyebrow">
            <span class="article-cat-tag">${esc(category)}</span>
            <span class="article-source-line">${esc(data.source || "Samuga Media")}</span>
            ${data.breaking || category === "BREAKING" ? `<span class="live-badge">Live</span>` : ""}
          </div>
        ...
        <div class="article-cta">
          <div class="article-cta-text"><strong>Follow this story on Samuga Community</strong><p>Live updates, breaking news, discussions.</p></div>
          <a class="header-btn primary" href="https://t.me/samugacommunity" target="_blank" rel="noopener">Join Telegram →</a>
        </div>

        <div class="share-bar">
          <div class="share-label">Share this story</div>
          <div class="share-buttons" role="group">
            <a class="share-btn" href="https://www.facebook.com/sharer/sharer.php?u=${encodeURIComponent(articleUrl)}" target="_blank" rel="noopener">Facebook</a>
            <a class="share-btn" href="https://t.me/share/url?url=${encodeURIComponent(articleUrl)}&text=${encodeURIComponent(title)}" target="_blank" rel="noopener">Telegram</a>
            <a class="share-btn" href="https://wa.me/?text=${encodeURIComponent(title + " " + articleUrl)}" target="_blank" rel="noopener">WhatsApp</a>
            <a class="share-btn" href="https://twitter.com/intent/tweet?text=${encodeURIComponent(title)}&url=${encodeURIComponent(articleUrl)}" target="_blank" rel="noopener">X</a>
  ...
  <footer class="site-footer" role="contentinfo">
    <div class="wrap footer-inner">
      <div class="footer-brand"><img src="/assets/SamugaNewsBot_Profile.png" alt="Samuga Media" loading="lazy"><div><div class="footer-brand-name">Samuga Media</div><div class="footer-tagline">Live news, powered by Samuga AI.</div></div></div>
      <nav class="footer-links"><a href="/">Home</a><a href="https://t.me/samugacommunity" target="_blank" rel="noopener">Community</a><a href="https://t.me/Samuga_Media" target="_blank" rel="noopener">Send a Tip</a></nav>
      <p class="footer-copy">© 2026 Samuga Media</p>
    </div>
  </footer>

  <script>
    window.addEventListener("scroll",()=>{const total=Math.max(document.body.scrollHeight,document.documentElement.scrollHeight)-innerHeight;const bar=document.getElementById("readingBar");if(bar)bar.style.width=(total>0?Math.min(100,scrollY/total*100):0)+"%";},{passive:true});
    document.getElementById("copyBtn")?.addEventListener("click",async()=>{try{await navigator.clipboard.writeText(location.href);const b=document.getElementById("copyBtn");b.textContent="✓ Copied!";setTimeout(()=>b.textContent="Copy link",1800)}catch(e){}});
  </script>
</body>
</html>`;
}
