...
async function articleApiFetch(path,options={}){
  let lastError=null;
  for(const url of [path,`${API}${path}`]){
    try{const response=await fetch(url,{...options});if(response.ok)return response;lastError=new Error(`HTTP ${response.status}`);if(response.status>=400&&response.status<500&&![404,408,429].includes(response.status))break}catch(error){lastError=error}
  }
...
function setupTheme() {
  document.querySelector("#themeToggle")?.addEventListener("click", () => {
    ...
    articleStorage.set("samuga-theme", next);
  });
}

async function loadSettings() {
  try {
    const response = await articleApiFetch("/api/site-settings", {cache: "no-store"});
    const data = await response.json();
    const settings = data?.settings || {};
    if (settings.community_url) {
      document.querySelector("#articleCommunity")?.setAttribute("href", settings.community_url);
    }
    if (!articleStorage.get("samuga-theme") && ["light", "dark"].includes(settings.default_theme)) {
...
async function loadArticle() {
  try {
    const response = await articleApiFetch(`/api/article?id=${encodeURIComponent(id)}`, {cache: "no-store"});
    const data = await response.json();
