...
function startContentLabRefreshTimer() {
  ...
  if (!$("#view-contentlab")?.classList.contains("active") || contentLabEditorLocks.size) return;
  contentLabRefreshTimer = setInterval(() => {
    if (document.visibilityState === "visible" && !contentLabEditorLocks.size) requestContentLabRefresh(false);
  }, 12000);
}
function pauseContentLabRefresh(key) {
  if (key) contentLabEditorLocks.add(String(key));
  clearInterval(contentLabRefreshTimer);
  contentLabRefreshTimer = null;
  const state = $("#contentLabConnection");
  if (state) state.textContent = "Editing — automatic refresh is fully paused until this editor is closed or saved.";
}
function resumeContentLabRefresh(key) {
  if (key) contentLabEditorLocks.delete(String(key));
  if (!contentLabEditorLocks.size) {
    startContentLabRefreshTimer();
    if (contentLabPendingRefresh) {
...
function contentLabSignature(items, history) {
  return JSON.stringify({
    items:(items || []).map(item => [item.key,item.title,item.summary,item.caption,item.dv_text,item.created_at,item.manual_only,item.review_type]),
...
async function loadContentLabImages(items) {
  await Promise.all(items.filter(item => item.has_card).map(async item => {
    try {
      ...
      if (media) media.innerHTML = `<button type="button" class="lab-card-image-button" title="Open full-size card"><img src="${url}" alt="Full Content Lab card for ${attr(item.title || item.key)}"></button>`;
