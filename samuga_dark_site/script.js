...
function cleanStory(s) {
  ...
  return {
    ...
    url: s.url || s.link || "#",
    image: s.thumbnail || s.web_image || s.image || s.card || s.image_url || FALLBACK_IMAGE,
    lang,
  ...
  return `
    <article class="story-card story-${s.lang}" dir="${s.lang === "dv" ? "rtl" : "ltr"}">
      <div class="story-topline">
        ...
        <span class="story-time">${escapeHTML(s.time)}</span>
      </div>
...
function updateStaticLanguageText() {
  ...
  if (activeLang === "dv") {
    if (latestTitle) latestTitle.textContent = "އެންމެ ފަހުގެ ޚަބަރުތައް";
    if (popularTitle) popularTitle.textContent = "ޕޮޕިއުލަރ";
    if (tipTitle) tipTitle.textContent = "ޚަބަރު ފޮނުވާ";
    if (tipText) tipText.textContent = "ސަމުގާ އޭއައި ބަލަންޖެހޭ ޚަބަރެއް އޮތްތަ؟";
    if (communityLink) communityLink.textContent = "ސަމުގާ ކޮމިއުނިޓީއަށް ޖޮއިން ވޭ →";
    if (footerText) footerText.textContent = "ސަމުގާ އޭއައިއިން ހިންގާ ލައިވް ޚަބަރު.";
    if (searchInput) searchInput.placeholder = "ޚަބަރު ހޯދާ";
  } else {
    if (latestTitle) latestTitle.textContent = "Latest Stories";
    if (popularTitle) popularTitle.textContent = "Popular";
    if (tipTitle) tipTitle.textContent = "Send a Tip";
    if (tipText) tipText.textContent = "Have a Maldives story for Samuga AI to watch?";
    if (communityLink) communityLink.textContent = "Join Samuga Community →";
    if (footerText) footerText.textContent = "Live news, powered by Samuga AI.";
    if (searchInput) searchInput.placeholder = "Search stories";
  }
}
