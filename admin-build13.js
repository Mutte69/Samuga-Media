...
function openView(name, options = {}) {
  ...
  if (name === "contentlab") { setContentLabLoading(); requestContentLabRefresh(true); contentLabRefreshTimer = setInterval(() => requestContentLabRefresh(false), 5000); }
...
async function requestContentLabRefresh(force = false) {
  ...
  if (contentLabInteractionActive()) {
    contentLabPendingRefresh = true;
    if (force) toast("Finish or close the Content Lab edit before refreshing.");
    return;
  }
...
async function runContentLabAction(card, action, corrected = "") {
  ...
  try {
    ...
    toast(data.message || `${String(key).toUpperCase()} action accepted`); await loadContentLab({force:true});
