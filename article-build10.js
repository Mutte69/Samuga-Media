...
function articleMarkup(article, url, dv) {
  const cover = article.cover_video
    ...
    : article.cover_image
      ? `<div class="article-cover article-cover-top"><img src="${attr(article.cover_image)}" alt="${esc(article.cover_caption || article.title || "")}" fetchpriority="high">${article.cover_caption ? `<p class="media-caption">${esc(article.cover_caption)}</p>` : ""}</div>`
      : "";
