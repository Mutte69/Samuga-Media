...
async function testArticleHandler(handler, path) {
  ...
  assert.match(html, /data-samuga-build="16\.2\.0"/);
  assert.match(html, /settings-sticky-header/);
  assert.match(html, /samuga-v3-shell-16-2-0\.js/);
  assert.match(html, /website-settings-runtime-16-2-0\.js/);
  assert.match(html, /website-settings-runtime-16-2-0\.css/);
  assert.match(html, /href="\/about"/);
  assert.doesNotMatch(html, /href="\/about\.html"/);
  assert.doesNotMatch(html, /samuga-v3-shell-16-0-1\.js/);
  assert.equal(response.headers.get("x-samuga-function"), "article-build16.2.0");
}

await test("dynamic /article route renders the 16.2 public shell", async () => testArticleHandler(articleRoute, "article"));
await test("dynamic /story route renders the 16.2 public shell", async () => testArticleHandler(storyRoute, "story"));
