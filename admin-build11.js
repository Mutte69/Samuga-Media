...
async function loadAnalytics() {
  ...
  try {
    ...
    const daily = data.daily || [], max = Math.max(1,...daily.map(item=>Number(item.views||0)));
    $("#analyticsTrend").innerHTML = daily.length ? daily.map(item => `<div class="analytics-day" title="${attr(item.day)}: ${item.views} views"><div><i style="height:${Math.max(4,Number(item.views||0)/max*100)}%"></i><b style="height:${Math.max(3,Number(item.visitors||0)/max*100)}%"></b></div><span>${esc(item.day.slice(5))}</span></div>`).join("") : `<div class="empty-panel">Traffic appears as public pages are viewed.</div>`;
    const pages = data.top_pages || [];
    $("#analyticsTopPages").innerHTML = pages.length ? `<div class="analytics-table-row head"><span>Page</span><span>Views</span><span>Visitors</span></div>` + pages.map(item => `<div class="analytics-table-row"><span><strong>${esc(item.label)}</strong><small>${esc(item.path||"")}</small></span><span>${Number(item.views||0).toLocaleString()}</span><span>${Number(item.visitors||0).toLocaleString()}</span></div>`).join("") : `<div class="empty-panel">No pageviews recorded yet.</div>`;
