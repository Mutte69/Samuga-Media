...
const SAMUGA_SITE_BUILD="15.9.5";
const FALLBACK_IMG="assets/SamugaNewsBot_Profile.png";
const FALLBACK_COVER="assets/samuga-article-placeholder.png";
const SPONSORS=[
...
function renderFeatured(){
  ...
  else if(s.cover_image){media.innerHTML=`<img src="${attr(s.cover_image)}" alt="" loading="eager" onerror="this.onerror=null;this.src=\'${FALLBACK_COVER}\'">`}
  else{media.className="media-placeholder";media.innerHTML=`<img src="${FALLBACK_COVER}" alt="Samuga Media">`}
}
...
function cardHTML(s){
  const cat=(activeLang==="dv"?CATS_DV:CATS)[s.category]||s.category;const media=s.cover_video?`<div class="card-media">${s.video_poster?`<img src="${attr(s.video_poster)}" alt="" loading="lazy" onerror="this.onerror=null;this.src='${FALLBACK_COVER}'">`:`<div class="video-poster-fallback" aria-hidden="true">▶</div>`}<span class="video-badge">▶ Video</span></div>`:s.cover_image?`<div class="card-media"><img src="${attr(s.cover_image)}" alt="" loading="lazy" onerror="this.onerror=null;this.src='${FALLBACK_COVER}'"></div>`:`<div class="card-media card-media-fallback"><img src="${FALLBACK_COVER}" alt="Samuga Media" loading="lazy"></div>`;
