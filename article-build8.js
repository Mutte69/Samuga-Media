...
function articleMarkup(article, url, dv) {
  const cover = article.cover_video
    ? `<div class="article-cover"><video src="${attr(article.cover_video)}" poster="${attr(article.video_poster || article.cover_image || "")}" controls playsinline preload="metadata"></video>${article.cover_caption ? `<p class="media-caption">${esc(article.cover_caption)}</p>` : ""}</div>`
    : article.cover_image
      ? `<div class="article-cover"><img src="${attr(article.cover_image)}" alt="${esc(article.cover_caption || article.title || "")}">${article.cover_caption ? `<p class="media-caption">${esc(article.cover_caption)}</p>` : ""}</div>`
      : "";
  ...
  return `
    <div class="article-container">
      <article class="article-story ${dv ? "article-dv" : "article-en"}" lang="${dv ? "dv" : "en"}" dir="${dv ? "rtl" : "ltr"}">
...
function relatedMarkup(items, dv) {
  ...
  return `<section class="related"><h2>${dv ? "ސަމުގާއިން އިތުރަށް" : "More from Samuga"}</h2><div class="related-grid">${items.slice(0, 4).map(item => `
    <a class="related-card" href="/article?id=${encodeURIComponent(item.id)}" dir="${DV_TEXT.test(item.title || "") ? "rtl" : "ltr"}">
